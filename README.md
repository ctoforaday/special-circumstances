...
| **gray-area** *(in design)* | the GCU shunned for reading minds | Trajectory evidence — what a session actually did, as against what it reported |
...
The plugin exists because the capability has already been built six times and hand-run twice, each time separately, each time against the same file format: bench integrity inspection, a record-join audit, per-seat context accounting, cost accounting, an attestation audit done by hand, friction mining done by hand, wall-clock forensics done by hand, and continuity across compaction done by hand. That is a plugin, not another script.

Two halves:

- **Mining.** Act-versus-claim discrepancy (a seat that says it verified something the trajectory shows it never touched), rework detection (the same tool against the same target, repeatedly), stalls and gaps from the timing, and the human-frustration surface in the user's own messages. Symmetric by construction — the same inspections aim at the bench as at the seats, because they run on the same record.
- **Continuity — the "Memento" problem.** When the context window fills, the harness compacts, and whatever the summary drops is gone. The agent leaves itself a note *before* the amnesia: the validation loop, the ordered next actions, the handles to work still running in the background. Hooks seal it deterministically and hand back only what the summary failed to carry. This half has been run by hand across six compaction boundaries and works; the plugin is what stops it being by hand.
