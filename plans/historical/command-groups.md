...
> Archaeology. The live design is `plans/command-groups.md`.
>
> **SUPERSEDED IN ITS CENTRAL THESIS, 2026-08 — "the tree is entities" was never built.**
> The eight entity groups below (`gap` · `evidence` · `document` · `direction` · `argument` ·
> `run`, plus `show` and `motion`) have no packages in `internal/cli/` and never had: no commit
> in this repository ever added `internal/cli/gap/`, `internal/cli/evidence/`,
> `internal/cli/document/` or `internal/cli/argument/` — checked with
> `git log --all -- '*internal/cli/gap*' …`, which returns nothing. What stage 6 actually shipped
> (#480, whose working commit reads "scope the command tree to the injected seat") kept the verbs
> FLAT and mounted them at the ROOT of a tree built for one seat
> — `mint`, not `gap mint`; `cite`, not `evidence cite`. The boundary is drawn by which commands
> exist for whoever is asking, so the entity node had nothing left to do. `motion` survived as
> the ONE group, because its subjects' contracts diverge in a way cobra cannot express with a
> flag; `show` survived as the projection group. Superseded by `plans/command-groups.md`, which
> describes the surface as built.
>
> The 2026-09-02 status markers below are kept exactly as they were written. They record the
> reversals that pass identified — the dual-read, the `avenue`/`direction` rename, `defaultFor`,
> `register`, `revision` — and they did NOT identify this one, which is why this document is the
> archaeology rather than the record of state.
>
> Everything below is preserved unedited, including the parts that later stopped being true.
> Path names in the group tables, the operator re-homing table and the declined-alternative
> sections are the DESTINATION this plan argued for, not paths the tool has.
