> Archaeology. The current design lives in [`plans/feov-run-injection.md`](../feov-run-injection.md); this file is the plan as written and the record of what changed.
...
## Addendum 2026-09-05 — what superseded each part (recorded at the split)

Written when this document was split from `plans/feov-run-injection.md`. It corrects nothing
above; it names where each part went, verified against the tree on that date.

| part of this plan | state now | superseded by |
|---|---|---|
| §1 `internal/seatenv` — `FEOV_RUN` → `--run` → inference, refusal on disagreement | **live**, and extended | `seatenv.VarWrapper` (`FEOV_RUN_FROM_WRAPPER`) added as a second carrier with a gate between them (#526) |
| §2 `PreOutcome(in, runDir)` — one entry point, deny arm first, `runDir` a parameter | **live**, `internal/hookgate/hookgate.go:246` | — |
| §2 `export …;` over an inline `VAR=x` prefix; single-quote escaping; control-character refusal | **live**, `injectEnv` | idempotency is now **per variable**, not whole-command (#510) |
| §2 match on invocation position; heredoc (`<<`) exclusion | **removed** | #510 — injection is unconditional on every Bash command in a live run; the prefix is prepended whole, so a heredoc body is byte-identical afterwards |
| §2 `cli/hook.go` reads the payload's `cwd`, resolves via `seat.InferRunDir` | **live**, but the file moved | `internal/hookcmd/hookcmd.go:99-103`, `cwdOf` |
| §3 miss observability at the seat | **CUT** at implementation (see Status above) | moot after #510: there is no longer a matcher to miss |
| §III `commands/research.md` gains `--bin-dir`; preflight FATAL on intent | **overtaken** | #282 — the version preflight runs unconditionally on every launch; `--bin-dir` now says WHERE the binary is, not WHETHER to check it |
| §III `cli.Version`/`recordToolVersion` → 0.35.0; `requirements.json` `[MODIFY]` | **stale** | versions moved many times since; `cli.Version` is `buildid.Revision()`; there is no `tools/requirements.json` in the tree (`recordToolVersion` survives only in `internal/cli/versionsync_test.go` and `commands/research.md`) |
| §III tracked `tools/feov-record` binary `[REBUILD]` | **deleted** | #293 (`3e5ffa33`) — already recorded above |
| §III/§V `internal/fuzz/prerewrite_fuzz_test.go`, `testdata/pretooluse-bash-feov.json` | **relocated / not built under those names** | the fuzz target is `internal/hookgate/inject_fuzz_test.go`; no `tools/testdata/` directory exists |
| §IV R10 (an invocation shape outside the enumerated positions is missed) | **moot** | #510 removed the enumeration |
| §V automated command list | **stale** | package paths moved (`internal/cli/hook.go` → `internal/hookcmd`), `scripts/golden`/`versionguard` etc. are repo-level and unchanged in spirit only |
| Appendix A, Class 2 — `roundRe` (`record/round.go:8`), `roleRe` (`record/findinglabel.go:12`), `Sprintf("R%d-%d")` (`record/replay.go:525`) | **still present** | `RoundOf` now returns `(round, known)` rather than a bare 0, and the ids it reads are validated by the roster gate |
| Appendix A, Class 2 — `view/changes.go` counting the prose substring `"estoppel —"` | **fixed** | #283 — `record.EstoppelRejections` counts the field; `Friction.kind` is a generated enum |
| Appendix A, Class 1 — `blue/frontier.md` | **gone** | #297 — frontier hypotheses became lines of inquiry, which carry an id |
| Appendix A, Class 1 — `blue/CHANGELOG.md` | **gone** | #251 — the round record is counted from `revision` events (`capture/capture.go:732`) |
| Appendix A, Class 1 — 57 markdown files of red's gap-pattern memory | **superseded** | `inputs/gap-patterns-by-class.json`, handed to a repairing seat by class (`commands/research.md:32`) |
| Appendix A, Resolution — `agent_id` as the collision-free identifier | **shipped** | #348 — `PreOutcome` injects `FEOV_AGENT_ID`; `internal/seatenv/identity.go` reads it; `FEOV_SEAT` and `FEOV_ROUND` were deleted as readers-with-no-writer |
| Appendix A — "`Event` is lowerCamel (`seatId`, `ts`)" | **FALSE as of the protobuf migration** | `internal/record/recordpb/record.proto` — the envelope is protobuf with `seat_id`, and the store is SQLite (`plans/record-sqlite.md`). `plans/record-protobuf.md` tracks this line as a `[MODIFY]` it had not yet made |
