...
- **The gate to hang that on already exists, and it is NOT `Event.schema_version`.**
  ...
  `requirements.json:3` (`"eventSchema": 2`) and refused at `internal/setup/setup.go:584-594` (the `got != expectSchema` branch; `:509-517` is the manifest read that supplies the expected value) —
