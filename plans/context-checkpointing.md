...
| **Validation check's trigger surface touched** | `FileChanged`, registered via `SessionStart`'s `watchPaths` | I2's *"reproduce, don't recall"* gains a **partial** mechanism. **Measured 2026-07-29** (`hook-surface-spike.md` §6): `watchPaths` takes literal paths only — relative or absolute, never globs, and a glob registers *nothing*, silently. A newly created file fires no event at all. So the check re-arms when an **already-existing** surface file is edited; the restore hook must expand the pattern to concrete paths itself, and that expansion is a snapshot. This narrows the agent's duty rather than removing it. |
...
| `hooks/filechanged-rearm.*` | hook (FileChanged) | **New; spiked 2026-07-29, not yet built.** Re-arm a validation check when its trigger surface is edited (I2). Input carries `file_path` and `event` (`change`), which is what the re-arm needs. The work is on the **restore** side: `watchPaths` accepts no globs and no directories, so `sc-checkpoint-restore` must expand each check's trigger surface into concrete existing paths and register those (`hook-surface-spike.md` §6). Files created later are invisible — a real limit, stated rather than discovered. |
...
- `watchPaths` re-arming of validation triggers (I2) is **spiked but unwired** — both events work and `FileChanged` carries `file_path` + `event`, but globs register nothing and new files never fire (`hook-surface-spike.md` §6), so the mechanism is narrower than §5 originally claimed.
