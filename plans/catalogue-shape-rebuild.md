...
| R10 | A hook stalls on the IMMEDIATE lock, or the extra read-only connection adds cost | low × low × low | `busy_timeout(5000)`; `BenchmarkOpen` reports `Open`'s cost on a current store (reported, not asserted) |
