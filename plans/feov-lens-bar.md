...
- `feov-record migrate` gains exactly one translation, and **it supplies a value ONLY where the source lacks the
  ...
  **The stated fill** (fork 1, gblock). Migrate already has a stated-miss mechanism for fields a record predates:
  ...
  (§V #2).
- **The material PASS gate is exempt under `Migrating`** (gblock, 2026-09-11). `record.go:1186-1189` wraps
  ...
  that did not exist when the PASS was issued. Measured: no archive exercises it. The three archived PASS gates
...
| fork (a) | `verify`'s `pass-closes-all-gaps` on a migrated archived PASS over a gap class defaults now make material | Accept the residue, stated in §IV. No mechanism is built. | gblock |
...
| An unmigrated run, or a hand-built fixture, breaks | no tolerance path | The open-time refusal names `migrate`; setup refuses a stale caller registry, naming the registry fix; `StageForRun` writes `by_grade`; #11 migrates all 13 archives with zero refusals. |
