...
- **`restart-recovery.md`** — in progress: after a restart, `telepathy agents` lists the sessions it cut off as `lost`, each with the commands that bring it back in the mode it had, and the `restart-recovery` skill walks a human through them. Makes the catalogue's store shape 5.
...
- **`dual-target-claude-antigravity.md`** — live design: concurrent Claude Code and Google Antigravity support from the same repository branch with zero regression. Covers the non-colliding manifest/hook directory layout (`.claude-plugin/plugin.json` vs `plugin.json`, `hooks/hooks.json` vs `hooks.json` symlink), `hookcore` stdlib payload ingestion, and skill visibility parity.
- **`hide-non-user-facing-skills-slash-commands.md`** — live design: unified skill visibility via dual frontmatter flags (`disable-slash-command: true` for Antigravity, `user-invocable: false` for Claude Code). Suppresses 25 non-user-facing procedural skills from the interactive `/` palette while keeping all 12 operator entry points and workflow skills visible.
