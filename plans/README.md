...
- **`mutation-audit.md`** — the live programme for `scripts/mutate`: its operational contract (a verdict per mutant, `-jobs`, the sandbox), the measured cost model, how to read a survivor count, and what remains unswept. Split out of [`historical/red-citations.md`](historical/red-citations.md) §V.7, which was filed as historical while this half was still being implemented from.
