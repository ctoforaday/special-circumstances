...
| **3. Instrumented reasoning** | Retain thinking summaries and **mine them for agent intent** — an explicit goal, not a tolerated side effect. **PARTLY DONE (2026-09-08):** `showThinkingSummaries` is now set box-wide and `sc-doctor` warns when it is not (#839); the mining side is unbuilt. Note `--thinking-display` no longer exists as a CLI flag at 2.1.260 — the settings key is the only lever | Summary text is present in seat transcripts **and** the miner returns it. The guard against summaries becoming evidence lives in **FEOV's citation ledger**, not here (gblock, 2026-09-08) — see T2 in `reasoning-telemetry.md` |
...
| G3 | **Summaries get promoted to evidence** because they are now available and readable. | Structural refusal **in FEOV, at the point of ruling** — its citation ledger refuses an unanchored closure and a summary anchors nothing (T2 in `reasoning-telemetry.md`). NOT in this plugin: the miner cannot know what a caller will do with a row, so a refusal here would be the convention T2 rejects, and two enforcement points for one rule is how they drift apart |
...
- No reading of thinking/summary content — Phase 3 owns that. Its guard is FEOV's, not this
  plugin's (see the Phase 3 row and G3).
