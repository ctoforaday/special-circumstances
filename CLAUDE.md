...
- BEFORE trusting a coverage number, YOU MUST remember what it cannot see: `internal/secrets` reported **100.0% of statements** while two of its eight secret patterns could be deleted outright with the suite green. Coverage measures REACH — `Scan()` loops over the pattern table, so every pattern is "executed" whether or not anything asks it to match. The question it cannot answer is whether a test would NOTICE. Ask it directly, on the code where a silently-wrong answer is the whole risk (the guards, the record's write path, anything that returns a plausible zero): delete a row of the table, invert the branch, and see whether anything fails. A field, a pattern or a rule that nothing asserts is one that can stop being written without a single test going red.
