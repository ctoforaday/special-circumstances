...
- **Versions move at a RELEASE BOUNDARY, not per PR.** An ordinary PR changes plugin content and leaves `version` alone. A release is a human call — made when the binary/text contract has actually moved — and it is ONE act: bump the plugin's `version` in `plugin.json` and tag that commit `<plugin>--v<version>`. The release job refuses a tag whose manifest disagrees (`versionguard -tag`), so the two cannot drift.
  - The per-PR bump rule that used to live here produced the opposite of what it promised: the version became a commit counter (1.45 → 1.57 in one afternoon) that told a consumer nothing, while the tags never moved — so no plugin had a tag matching its own version and `sc-doctor -fix` pinned every download to a release that does not exist (#405).
  ...
  - `versionguard` still fails a version that goes BACKWARDS — that arm caught two real concurrent bumps this month — and still sweeps for stale per-binary version constants.
