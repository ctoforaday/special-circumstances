...
- BEFORE the verdict, YOU MUST list every finding that fails that test under NOTES and still PASS — test layout, exact assertions, which test kills which mutant, a census's stated count (not an omission — see Completeness), line numbers, wording, names, placement, and any break a gate fails LOUDLY (compile, an existing test, a golden diff, a CI leg, the first run of §V). Findings of this kind are common, and a whole FAIL round may hold nothing else; the defects that reach main are the silent kind.
- During the audit, YOU MUST state a gap as its **CLASS**, with the sweep that covers every instance, and as the **PROPERTY** that must hold — not a mechanism you have not audited; a prescribed mechanism is itself unaudited design. YOU MUST check it on the code path the plan ships, never on a probe you built beside it — a property certified against a probe's own configuration says nothing about the one the code builds.
...
- Worth-the-cost is not a FAIL class, and not yours to decide: YOU MUST list under FORKS, on PASS or FAIL, every decision whose options are "build X" or "accept residue Y, stated" — a fix that adds machinery, a guarantee that could be best-effort, and a plan that bundles independently shippable parts without a value case for the whole ("split it and ship the cheap parts first" | "keep it whole, because <value case>"), and a §I value case that is missing, or does not show the change beating the naive version by more than its machinery costs, bundled or not ("build <the machinery>" | "ship the naive version, stated"). That decision is the human's.
