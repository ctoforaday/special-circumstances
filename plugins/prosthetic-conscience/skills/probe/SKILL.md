---
name: probe
description: Harness-spike probe (Phase 1) — verify prosthetic-conscience's skill/agent/entry-point/hook wiring and the communication model are live.
---

Run the Phase 1 harness probe. Report PASS/FAIL per primitive. Model terse-communication: no filler.

1. **Entry point** — confirm this skill loaded from `prosthetic-conscience` (if you are reading it, it did).
...
5. **Report** a table: skill / agent / entry point / hook / voice → PASS or FAIL, one terse line each. Be explicit about FAILs — this probe surfaces breakage, it does not reassure.
