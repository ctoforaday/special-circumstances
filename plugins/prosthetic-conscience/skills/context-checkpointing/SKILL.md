...
- AFTER a turn-boundary line reports the context heavy or the note stale — or when you can see the session has grown long — YOU MUST write or refresh the note THEN, before anything else, and then tell the human in one line, with the figure the line gave, and propose a moment to compact or start a fresh session. Write first because the human may not be there: an automatic compaction with a note on disk loses nothing a resume needs, and one without it is the failure this clause exists for. When nothing is in flight — no pending work, no running handles — say so instead of writing an empty note.
- YOU MUST NOT compact, or end the session, on your own: shedding the context drops reasoning no note carries, so when to do it is the human's call. A warning only the agent heard saves nothing.
