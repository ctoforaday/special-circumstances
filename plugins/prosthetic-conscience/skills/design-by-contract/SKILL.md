...
- **One agent, one responsibility**: BEFORE defining an agent, YOU MUST keep it single-purpose; YOU MUST NOT bloat one agent with multiple distinct responsibilities — decompose instead.
...
- **A script must do what a script can do**: BEFORE writing a multi-step instruction for an agent to execute, YOU MUST check whether the step has a computable correct answer from observable state — if it does, it belongs in a tested script/binary the instruction *invokes*, never in prose the agent *interprets*. Prose is for decisions (weighing tradeoffs); prose-executed mechanics are an unenforced good-faith contract, and an unenforced contract is the failure class behind lost staged input, skipped setup steps, and silent capture omissions. For promises about FUTURE behavior (freezes, "never do X while Y"), YOU MUST record the commitment as observable state (a marker file) and let a deterministic guard consult it — memory is not an enforcement mechanism.
...
- BEFORE shipping a change to a RULE — a skill, a seat prompt, a constitution, a
  ...
  seat, where it passes the very tests your patch added — because those tests were
  written about the instance too. Classes and the minting
  ...
  instance"); this clause names the RULE surface and points at a mechanism,
  because a duty scoped to neither is one nothing checks.
