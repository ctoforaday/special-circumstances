...
- During error or lint repair, YOU MUST NOT attempt the same fix more than 3 times; AFTER the third consecutive failure, YOU MUST stop, summarize what was tried, and escalate to the human. A `PostToolUseFailure` hook counts this deterministically (same tool, same target, three failures inside a window) — but it counts TOOL failures only. A command that exits 0 while the work still failed, an edit that applies cleanly and does not fix the bug, a test runner reporting failures in its output: none of these reach it. **The counter's silence is not evidence the loop was broken** — the judgement half of this rule is still yours.
