...
Command groups — each holds commands THIS page does not list:
  line-of-inquiry a direction the report could take, and what became of it — `propose` one, `move` one you already proposed
...
(The sentence is found in the rep…) → SHARED §2
...
OCR-DERIVED TEXT CAN MISREAD. A leaf citation of it names the span you quote from the source's reading, verbatim, and the tool records the PDF page that span sits on — never type a page. The span must sit whole on one page of the reading; the refusal says which fix applies when it does not.

THE TITLE IS PRINTED IN THE REPORT, in the source's note and its Bibliography entry; the tool adds the PDF page to the note of a quote from OCR text. Give the source's own name — author, work, section — for a reader of the SUBJECT, not an account of how you found or read it: no "this sitting" or "this run", no "re-read at the leaf". How much of the source you read is recorded as its own field, not in the title's words. Where that reading matters to a claim, say it in the report's prose, in the subject's terms: "the source, read in full, states…", "the abstract claims, and the body does not show, …". The tool flags a title that sounds like the run talking about itself; it refuses nothing, because a real source's own title can.
...
It renders as this sitting's ### BLUE CLOSING section, one entry per docketed gap.
...
(CORRECTING WHAT YOU RECORDED. If…) → SHARED §8
...
A raw write or a shell edit of the report is denied.

TAKING RED'S FIX AS IT STANDS. Where red prescribed concrete replacement text on a gap, you can
apply that text instead of retyping it. Your argument for the change is still required and still
yours — accepting is a decision, not a transcription.

Retyping it is the same act only if every byte matches: a stray space or a re-wrapped line records
that you did NOT take red's text, and costs you the estoppel that stops red re-opening its own words.
...
(WHAT MAY BE IN THE REPORT: resea…) → SHARED §9
...
Flags:
      ...
      --answers gap-id   the gap id this edit responds to (G4) — the provenance join key; omit only for an edit that answers no gap
  -h, --help             help for edit
      --key string       → SHARED §10
...
Flags:
  ...
  -h, --help          help for fetch
      ...
      --via backend   reach the source through a named backend: live | archive | oa | metadata | arxiv | eric | auto. They answer DIFFERENT questions — live the URL itself (the same as omitting this); archive what the page said on a date (right for web pages, usually the landing page for a subscription article); oa whether a legal open copy exists; metadata only that the source exists and where (no text, and the honest answer when there is none to get); arxiv the preprint's PDF or LaTeX source; eric the US education index's record, with its full text where ERIC holds an authorised copy; auto tries arxiv, archive, oa, metadata in that order. A refused live fetch falls back through that same order
...
(Global Flags:) → SHARED §7
==============================================================================
$ feov-record line-of-inquiry move --help
say what became of a direction you proposed — every open line owes this before your sitting ends

An open line — proposed or pursued — owes this before your sitting ends; re-recording `pursued` with what you learned is a REAFFIRMATION and settles it for this sitting.

The fate is not the point; the account of what changed it is — a line abandoned against its own stated hypothesis is evidence of choosing, one abandoned on a shrug is not.

THE REASON IS PRINTED IN THE REPORT, beside the line. Give the account in the SUBJECT'S terms — what was found about the question that settled this fate — not the steps you took to find it. No provenance or attribution tags, no notes to another seat, no narration of the run. "The trial-division bound is 9, so no subtler test is needed" is a reason; "fetched and read the sources at the leaf, then grepped the repo" is a diary. What you did, in order, is already on the record.
...
Usage:
  feov-record line-of-inquiry move [flags]

Flags:
      --as as-value             REQUIRED — the fate of this line of inquiry
      --correction-why string   → SHARED §5
      --corrects string         → SHARED §6
  -h, --help                    help for move
      --id inquiry-id           the line of inquiry whose fate you are moving (Q1, Q2 …); the lines-of-inquiry projection lists every one
      --reason string           the account of what settled this fate, in the SUBJECT's terms — printed in the report beside the line
...
(Global Flags:) → SHARED §7
==============================================================================
$ feov-record line-of-inquiry propose --help
put a direction on the record, when something is worth trying and a later run needs to know you chose it
...
You may propose one AT ANY POINT in a run, not only at the start: a direction that occurs to you in your third sitting is exactly the one a later run needs.

THE APPROACH YOU PROPOSE IS THE LINE, AND IT IS PRINTED IN THE REPORT word for word, under Research areas, Future research directions or Alternatives considered. So write it as research prose for a reader of the SUBJECT — the question the direction asks, not a log of what you did to ask it. No provenance or attribution tags, no notes to another seat, no narration of the run: "whether 91's factorisation follows from a divisibility test short of trial division" is a line, "ran this lane's slice against Q2 and grepped the repo" is a diary. Who moved it, the path it took and red's ruling are already on the record, and ship in lines-of-inquiry.md.
...
Usage:
  feov-record line-of-inquiry propose [flags]

Flags:
      --correction-why string   → SHARED §5
      --corrects string         → SHARED §6
  -h, --help                    help for propose
      --hypothesis string       what would be TRUE if this line pays off — the claim a later abandonment is judged against, so the fate is checkable rather than a shrug
      --method string           the source class or technique it belonged to, when that is what distinguishes it
      --reason string           the line — research prose for a reader of the SUBJECT, printed in the report word for word
...
It goes on the RECORD, not in the envelope: capture scores it from there, and it reaches a reader through the BOARD — `docket.md` carries your manifest, and a gap you repaired with no row is named there as a repair nobody audited. It is not in the research report, which carries what the checking showed, not the apparatus. A gap the BENCH disposed of owes no receipt: nobody repaired it.
...
A FAILING CHECK IS A RESULT: a non-zero exit is recorded as evidence, not as an error. Disproving your own claim is worth as much as confirming it.

YOUR NOTE IS PRINTED IN THE REPORT, as this computation's footnote and as its heading in evidence.md. So write it for a reader of the SUBJECT: what the computation establishes, not what happened while you ran it. No "this sitting" or "this run", no "rather than trusting the lane draft", no gap ids (the gap it settles is recorded as its own field), no notes to another seat. That it was computed, and whether it reproduced, the tool already says beside your note. Where the kind of evidence still matters to the claim, say it in the subject's terms: "a reproducible computation shows 7 divides 91", "trial division by every prime up to 9 finds the divisor 7", "a second, independent implementation of modular exponentiation gives the same three residues". The tool flags a note that sounds like the run talking about itself; it refuses nothing.
...
OUTPUT (JSON): {sources:[{anchor,url,title,sha256,access_date,location,text,seat_id,epoch,source_text_origin,ocr_quote,pages:[number],ocr_engine,ocr_text_sha,corroborated_by,verified:[{claim,anchor,label,outcome,confidence,text,url,title,access_date,seat_id,epoch,page,page_render_sha,reading_render_sha}]}],proofs:[{anchor,sha256,basis,cites,drift,seat_id,epoch,verified:{reproduced,sound,note,seat_id,epoch,struck:{…}},struck_reruns:[{reproduced,sound,note,seat_id,epoch,struck:{…}}]}],independent:[{claim,anchor,label,outcome,confidence,text,url,title,access_date,seat_id,epoch,page,page_render_sha,reading_render_sha}],reopened:[string],unanswered_contradictions:[string],counts:{sources,proofs,proofs_unverified,sources_unverified,sources_refuted,verifications}}
