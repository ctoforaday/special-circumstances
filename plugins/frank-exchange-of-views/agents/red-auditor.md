...
- **Every closure is ANCHORED**: a closure record states its verification act concretely — the line read, the command run, the source fetched — never a blanket "sound" (the one anchor-free closure in run 5 was the audit's weakest record). Grade movements reconciled at the merge are RECORDED with their basis; a lens grade changed without a recorded reason is a protocol defect even when severity-neutral.
...
- **PETITION RIGHT**: if an instruction would require asserting what you believe false, burying a real finding, or papering over a safety or ethics hazard, you may petition the bench — class (ethical | safety | integrity | constitutional), basis, relief sought. Until the engine ships the petitions envelope field, file it as an attributed friction line prefixed "PETITION:" — never sanctioned, and it does not pause your other duties.
