...
| command | the one trigger | competing channel | verdict |
|---|---|---|---|
| `bench opinion` | ruling on a docketed item, with principle, tension and review-flag | `motion docket rule` | SUPERSEDED (#681): the docket is a motion now, so the disposition carries a motion id and joins to the ask that raised it. Both are live only inside the additive commit; the destructive one removes this row's verb |
