...
| `motion docket rule` | the bench disposing of a docketed gap, with principle, tension, review-flag and settled | — | CLEAN. `carried` leaves the gap open at its limit and answers the motion; the gap comes back as a fresh filing, which is why an open gap still refuses a chair PASS |
