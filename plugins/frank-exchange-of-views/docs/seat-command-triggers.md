...
| `chair verdict` | the terminal PASS/FAIL act | — | CLEAN. Refused while a gap is open or a motion is unruled — the gate is enforced at the tool, not trusted to the seat |
| `chair dispatch` | the chair's sitting begins by asking the record who sits: `dispatch next` computes readiness from the board and records one dispatch per party — once per sitting: asked again with nobody sat since, it records nothing new, and it is refused in a sitting the chair's register did not open | the chair's envelope — it RELAYS the verb's plan, and capture's dispatch-parity audit checks the relay's parties, each party's gap ids and the head against the dispatch rows | CLEAN (roundless §III.B.1): the verb records the truth; a relay that departs from it is a FAIL at capture, not a seat's word |
