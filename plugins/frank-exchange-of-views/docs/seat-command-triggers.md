...
|---|---|---|---|
| `lens register` `chair register` `blue register` `bench register` | first action of EVERY sitting, a resumed session's included — the record counts sittings by registers | — | CLEAN: a dispatched seat that has not registered is readied again; a chair that has not is refused `dispatch next` |
| `lens log` `chair log` `blue log` `bench log` | an entry for the operator, typed: a defect, a request, an impediment worth noting, or `--type nominal` for a clean sitting | — | CLEAN. Every seat WRITES it; the READ is the operator's (`feov-record log`), because a capability gap is a report to the human who can retool the seat, not material for the debate |
| `lens show` `chair show` `blue show` `bench show` | read a projection | — | CLEAN (read path). The projections are their own vocabulary with their own gate — see `TestEveryViewNamesTheVerbThatFillsIt` — so the `show <view>` subtree is not enumerated here |
| `bench inquest debate` `chair inquest debate` | ruling a docketed gap or a motion, where what each side ARGUED decides it | — | CLEAN (read path, adjudicators only). The bench and the chair both rule between two parties and take neither one's account, so they read the transcript itself |
| `bench inquest motions` `chair inquest motions` | ruling a motion, or checking what was already formally contested and how | `show work` lists the bench's UNRULED motions as items; this is the ask and the ruling in the filer's own words | CLEAN (read path, adjudicators only). The chair rules GRADE motions and the bench the rest, so both need the ask in the filer's words |
| `bench inquest telemetry` `chair inquest telemetry` | judging whether the debate is still converging — the STOPPING question | — | CLEAN (read path, adjudicators only). A trend across epochs, not a snapshot |
...
| `lens verify` | you checked a citation BLUE authored against its source | envelope `corroboration[]` re-typed claim/reference/confidence | EXECUTED (#326/#382): the envelope carries no corroboration array, and blue reads what red found from `show evidence`. The verb carries BOTH axes — `--as` what the source did (with its negative half: `refutes`, `absent`) and `--confidence` how sure you are of that — and `--anchor` naming which citation is REQUIRED, because a source blue never cited is `corroborate`'s. A citation with pages (an OCR quote) also needs `--page`, the page image checked, unless the outcome is `unreachable` |
...
| command | the one trigger | competing channel | verdict |
|---|---|---|---|
| `lens mint` | a defect worth putting on the board | `RED_ENVELOPE.gaps[]` re-typed every open gap's prose every sitting | EXECUTED: the envelope carries routing refs and the board is read back with `show worklist` |
| `lens close` | a gap answered, with an anchor naming who checked what with which tool — the ORIGINATOR closes (roundless §III.B.3); another seat's close is refused at the record | — | CLEAN |
| `lens regrade` | change a gap's grade in place | debate.js prose "apply the new grade in the ledger" never named the verb, so a seat may re-mint or edit instead | EXECUTED (#325, verified 2026-08-13): named in debate.js and in red-auditor.md. The 2026-08-12 elicitation then found a chair seat listing it unprompted — *"Regrade R1-1 without closing it — lower severity, keep it open until blue actually edits"* — which is the move from *never perceived* to *weighed* the naming was for |
| `lens near-match` | before minting: is this gap already on the board, open or closed? | the archive read the lens would otherwise do by hand — and the screen the chair did by eye before lenses minted (roundless §III.B.3) | CLEAN (read-only, records nothing). Same elicitation result as `regrade` — unnamed in the prompts, unlisted by the seats |
| `chair spot-check` | the chair sitting's archive spot-check duty | the envelope array is deleted | RESOLVED (#317): the verb is the single channel and its floor is computed from the board |
| `chair position` | the sitting's RED narrative | a hand-written `### RED` section in debate.md | EXECUTED: the transcript is rendered from `position` events |
| `chair closing` | a closing argument on a docketed item | — | CLEAN |
| `lens class list` | needing to know what classes exist — before minting against one, and before coining a new one | reading the checked-in `feov-memory/class-registry.json` or the generated Go source, OUTSIDE the run directory | EXECUTED: the coining check refuses a class the registry does not know, and nothing on any seat's surface could say what it knows, so the only way to answer was to leave the record tool — the one place a seat is told it should never need to go |
| `lens class new` | coining a gap class the registry does not have | ~~`mint --class-new`~~ | EXECUTED: it was four flags on `mint` (a boolean whose whole meaning was "I also passed three other flags") writing a second event that had nothing to do with putting a gap on the board |
| `chair carry` | restating a closure an earlier sitting already made | ~~`close --carried-from`~~ | EXECUTED: a carry needs no verification triple, is exempt from `--reason` and from the open-gap check, and must name a real prior closure — four branches in `validate` keyed off one flag, so one verb could require nothing |
| `chair verdict` | the terminal PASS/FAIL act | — | CLEAN. A PASS is refused while a material gap is open, a lens is ready, a stale area is unnamed or a motion is unruled — the gate is enforced at the tool, not trusted to the seat |
| `chair dispatch` | the chair's sitting begins by asking the record who sits: `dispatch next` computes readiness from the board — a lens by its retirement state — records one dispatch per party, and names the stale areas the chair spot-checks before a PASS — once per sitting: asked again with nobody sat since, it records nothing new, and it is refused in a sitting the chair's register did not open | the chair's envelope — it RELAYS the verb's plan, and capture's dispatch-parity audit checks the relay's parties, each party's gap ids and the head against the dispatch rows | CLEAN (roundless §III.B.1): the verb records the truth; a relay that departs from it is a FAIL at capture, not a seat's word |
...
| command | the one trigger | competing channel | verdict |
|---|---|---|---|
| `blue edit` | any change to `blue/report.md` after synthesis | a raw Write/Edit to the file | EXECUTED — the lockdown: report.md is read-only to a response seat and the tool refuses an edit that drops, duplicates or invents an anchor |
| `blue ingest` | freeze the synthesized report into the record, once, by its author at synthesis | ~~a raw write leaving a file behind~~ | EXECUTED (#709): reads the file verbatim into a `BaseIngest` event, PROVES the record renders it back byte-for-byte, then DELETES the file — after which the report is the base plus its diff-stack and there is no file to write, chattr, or bypass. Write-once (a second ingest is refused → `blue edit`) and author-only |
...
| `blue revision` | the sitting record: what changed this sitting | ~~hand-written `blue/CHANGELOG.md`~~ | EXECUTED (#251): the file is gone from `setup`, from every prompt and from both blue constitutions. The event was always the canonical half — capture counted it, the report renders it as the revision history — while the prompts demanded the file, so the audit read the channel nobody was told to write |
| `blue manifest-row` | the self-audit receipt for a repaired gap | envelope `manifest[]` array | RESOLVED (#318): the verb is the single source, the row is on the record, and a REPAIRED gap with no row is named in the report as a repair nobody audited. Narrowed from "closed" with the manifest predicate: a bench-disposed gap was never repaired, so no receipt is owed. Narrowed again to OWED gaps (#868): blue dispatched onto it, it still open when blue registered, and blue's edit that sitting answering it (`record.ManifestOwed`). A gap its lens closed before blue sat, or one blue rebutted without an edit, owes nothing; the report, the scorecard and blue's work list all read that one predicate. A row that came out wrong is corrected in its sitting by the same verb with `--corrects <key>`; the struck row stays, shown struck, and is not counted |
| `blue position` | the sitting's BLUE narrative | a hand-written `### BLUE` section | EXECUTED |
...
| `motion docket file` | an open gap you cannot settle yourself and the bench should | — | CLEAN. Filed by ANY seat — the chair because it owns what reaches the bench, blue to escalate a gap it believes red is wrong about |
| `motion docket rule` | the bench disposing of a docketed gap, with principle, tension, review-flag and settled | — | CLEAN. `remanded` leaves the gap open at its limit and answers the motion; the gap comes back as a fresh filing, which is why an open material gap still refuses a chair PASS |
...
| **regrade → canonical; debate.js must name it** | EXECUTED (#325, verified 2026-08-13). Named in debate.js and red-auditor.md, and a probed chair seat listed it among its options unprompted. |
