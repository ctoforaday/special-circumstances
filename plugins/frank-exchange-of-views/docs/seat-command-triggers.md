...
| command | the one trigger | competing channel | verdict |
|---|---|---|---|
| `lens mint` | a defect worth putting on the board | `RED_ENVELOPE.gaps[]` re-typed every open gap's prose every round | EXECUTED: the envelope carries routing refs and the board is read back with `show worklist` |
