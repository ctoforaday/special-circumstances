...
|---|---|---|---|
| `lens register` `merge register` `blue register` `bench register` | first action of EVERY sitting, a resumed session's included — the record counts sittings by registers | — | CLEAN: a dispatched seat that has not registered is readied again; a chair that has not is refused `dispatch next` |
...
| `merge spot-check` | the chair sitting's archive spot-check duty | the envelope array is deleted | RESOLVED (#317): the verb is the single channel and its floor is computed from the board |
| `merge position` | the sitting's RED narrative | a hand-written `### RED` section in debate.md | EXECUTED: the transcript is rendered from `position` events |
...
| `merge carry` | restating a closure an earlier sitting already made | ~~`close --carried-from`~~ | EXECUTED: a carry needs no verification triple, is exempt from `--reason` and from the open-gap check, and must name a real prior closure — four branches in `validate` keyed off one flag, so one verb could require nothing |
...
| `merge dispatch` | the chair's sitting begins by asking the record who sits: `dispatch next` computes readiness from the board and records one dispatch per party — once per sitting: asked again with nobody sat since, it records nothing new, and it is refused in a sitting the chair's register did not open | the chair's envelope — it RELAYS the verb's plan, and capture's dispatch-parity audit checks the relay against the record | CLEAN (roundless §III.B.1): the verb records the truth; a relay that departs from it is a FAIL at capture, not a seat's word |
...
| `motion docket rule` | the bench disposing of a docketed gap, with principle, tension, review-flag and settled | — | CLEAN. `carried` leaves the gap open at its limit and answers the motion; the gap comes back as a fresh filing, which is why an open gap still refuses a merge PASS |
