...
argument-hint: <topic> [--lanes N] [--lenses N] [--max-rounds N]
---

Run a frank exchange of views on the topic in `$ARGUMENTS` (if no topic given, ask — do not guess). Parse optional flags: `--lanes` (blue candidate drafts, default 3), `--lenses` (red audit passes, default 3), `--max-rounds` (safety ceiling only — the real terminator is red-PASS or judged deadlock; default 12).
...
2. Invoke the **Workflow** tool with `scriptPath` = `${CLAUDE_PLUGIN_ROOT}/skills/gold-standard-research/scripts/workflow.js` and `args` = `{ "topic": "<topic>", "runDir": "<run directory path>", "lanes": N, "lenses": N, "maxRounds": N }`.
