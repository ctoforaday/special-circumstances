...
func initEvent(paths ...string) string {
	var entries []string
	for _, p := range paths {
		entries = append(entries, `{"name":"frank-exchange-of-views","path":"`+p+`","version":"1.70.0"}`)
	}
	return `{"type":"system","subtype":"init","plugins":[` + strings.Join(entries, ",") +
		`,{"name":"prosthetic-conscience","path":"/elsewhere/prosthetic-conscience","version":"0.47.0"}]}`
}
...
func TestPluginLoadedFromRefusesTheInstalledRelease(t *testing.T) {
	...
	want := t.TempDir()
	p := writeTrajectory(t, initEvent(installed))
...
func TestPluginLoadedFromResolvesTheDirectoryBeforeComparing(t *testing.T) {
	...
	p := writeTrajectory(t, initEvent(real))
