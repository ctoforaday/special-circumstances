...
var hookLocalPackages = map[string][]string{
	"./cmd/feov-pretooluse": {
		...
		// hookfailures, so a failure this hook swallows reaches a human (generated by
		// scripts/hookfailgen; plans/hook-failures-reach-someone.md).
		//
		// THE REASON IT IS WORTH A LINE HERE: every hook here exits 0, and a hook's stderr at exit 0
		// reaches the client's debug log and nobody — measured 2026-09-16. Panics, a turn limit the
		// record could not hold, a marker that cannot be read: each was a stderr line or nothing.
		// PreToolUse is this plugin's ONLY displaying event, so this binary is the one that says what
		// the sitting hooks recorded.
		//
		// THE COST, STATED: standard library only (encoding/json, os, path/filepath, sync, time and
		// their dependencies, which hookcmd already linked), no package-level state, no init(). Per
		// firing it is one stat of the record file — ENOENT on a healthy machine — and nothing written.
	...
	"./cmd/feov-subagentstop": {
		"internal/feov", "internal/seatenv", "internal/hookgate", "internal/runlive", "internal/hookcmd",
		"internal/sittinghook", "internal/buildid", "internal/sittingcap",
		"internal/hookfailures", // as feov-subagentstart
	},
}
