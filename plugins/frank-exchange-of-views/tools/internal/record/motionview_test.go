...
func TestMotionsViewCarriesTheAskNotJustTheAnswer(t *testing.T) {
	...
	if _, err := Append(runDir, "blue-respond-r1", "motion", NewPayload().
	...
	if _, err := Append(runDir, "red-merge-r1", "motion-rule", NewPayload().
...
func TestThePassRefusalNamesTheRead(t *testing.T) {
	...
	for _, sid := range []string{"blue-respond-r1", "red-merge-r1"} {
		if _, _, err := RegisterSeat(runDir, sid); err != nil {
			t.Fatal(err)
		}
	}
	if _, err := Append(runDir, "red-merge-r1", "mint", NewPayload().
