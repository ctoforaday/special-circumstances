...
// evidenceBoard builds the event stream, the way the other view tests do — the projection
// is a pure function of the event list, so the test states the events and nothing else.
func evidenceBoard(events ...*Event) []*Event { return events }
...
func TestEvidence_CitationAnchorResolvesToItsSource(t *testing.T) {
	b := evidenceBoard(evidenceEvent(t, 1, "blue-r1", &recordpb.Cite{SourceTextOrigin: recordpb.SourceTextOrigin_SOURCE_TEXT_ORIGIN_EMBEDDED.Enum(), WorkStatus: recordpb.WorkStatus_WORK_STATUS_STANDING.Enum(),
...
func TestEvidence_RedLensCiteIsNotASource(t *testing.T) {
	b := evidenceBoard(
		evidenceEvent(t, 1, "lens-r1", &recordpb.Cite{SourceTextOrigin: recordpb.SourceTextOrigin_SOURCE_TEXT_ORIGIN_EMBEDDED.Enum(), WorkStatus: recordpb.WorkStatus_WORK_STATUS_STANDING.Enum(), Text: proto.String("checked something")}),            // no label: red's
		evidenceEvent(t, 1, "blue-r1", &recordpb.Cite{SourceTextOrigin: recordpb.SourceTextOrigin_SOURCE_TEXT_ORIGIN_EMBEDDED.Enum(), WorkStatus: recordpb.WorkStatus_WORK_STATUS_STANDING.Enum(), Label: proto.String("c-1"), Url: proto.String("u")}), // blue's
	)
	got := EvidenceJSONOf(b)
...
func TestEvidence_VerificationAttachesToItsCitation(t *testing.T) {
	b := evidenceBoard(
		evidenceEvent(t, 1, "blue-r1", &recordpb.Cite{SourceTextOrigin: recordpb.SourceTextOrigin_SOURCE_TEXT_ORIGIN_EMBEDDED.Enum(), WorkStatus: recordpb.WorkStatus_WORK_STATUS_STANDING.Enum(), Label: proto.String("c-1"), Url: proto.String("https://example.org/p")}),
...
func TestEvidence_UncheckedSourceSaysSoInTheJSON(t *testing.T) {
	b := evidenceBoard(evidenceEvent(t, 1, "blue-r1", &recordpb.Cite{SourceTextOrigin: recordpb.SourceTextOrigin_SOURCE_TEXT_ORIGIN_EMBEDDED.Enum(), WorkStatus: recordpb.WorkStatus_WORK_STATUS_STANDING.Enum(), Label: proto.String("c-1"), Url: proto.String("u")}))
	out, err := json.Marshal(EvidenceJSONOf(b))
	if err != nil {
		t.Fatal(err)
	}
...
func TestEvidence_RefutedCitationsAreCounted(t *testing.T) {
	for _, outcome := range []recordpb.SourceOutcome{
		...
		b := evidenceBoard(
			evidenceEvent(t, 1, "blue-r1", &recordpb.Cite{SourceTextOrigin: recordpb.SourceTextOrigin_SOURCE_TEXT_ORIGIN_EMBEDDED.Enum(), WorkStatus: recordpb.WorkStatus_WORK_STATUS_STANDING.Enum(), Label: proto.String("c-1"), Url: proto.String("u")}),
