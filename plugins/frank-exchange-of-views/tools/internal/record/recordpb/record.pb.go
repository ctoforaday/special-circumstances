...
//
// FIELD NUMBERS ARE INERT HERE, AND NAMES ARE WHAT BREAK. This file is a SCHEMA DEFINITION
// LANGUAGE, not a wire format: nothing in this tree marshals or unmarshals binary protobuf.
// `proto.Marshal` appears exactly once, at `recordsql/store.go` as `var _ = proto.Marshal` — an
// import-keeper, not a call. Records live in SQLite and leave through protojson, and BOTH are
// keyed on NAMES: `recordsql.TableName` is `snake(message.Name())`, every column is
// `field.Name()`, every enum table is `"enum_" + snake(enum.Name())`, and foreign keys reference
// `field.Name()`. Search this file for a field number's effect on storage and there is none.
//
// So the rule that used to stand here — "field numbers are permanent, a renumber silently
// reinterprets every record already written" — described the textproto/binary era the SQLite
// store retired. Renumbering costs nothing. RENAMING is the breaking change: rename a message
// and the table moves; rename a field and the column moves; and a reader pointed at a record
// written before the rename finds the old name absent and returns ZERO ROWS, which is
// byte-identical to a clean board. That is the failure to design against, not renumbering.
//
// THIS HOLDS ONLY WHILE NOTHING WRITES THE WIRE FORMAT. If binary marshalling is ever
// introduced, field numbers become load-bearing again and the permanence rule comes back with
// them. Say so here if that day comes.
...
// CorrectionTier is how much of an act a same-sitting correction may change.
type CorrectionTier int32

const (
	CorrectionTier_CORRECTION_TIER_UNSPECIFIED CorrectionTier = 0
	CorrectionTier_CORRECTION_TIER_NONE        CorrectionTier = 1
	CorrectionTier_CORRECTION_TIER_PROSE       CorrectionTier = 2
	CorrectionTier_CORRECTION_TIER_FULL        CorrectionTier = 3
)

// Enum value maps for CorrectionTier.
var (
	CorrectionTier_name = map[int32]string{
		0: "CORRECTION_TIER_UNSPECIFIED",
		1: "CORRECTION_TIER_NONE",
		2: "CORRECTION_TIER_PROSE",
		3: "CORRECTION_TIER_FULL",
	}
	CorrectionTier_value = map[string]int32{
		"CORRECTION_TIER_UNSPECIFIED": 0,
		"CORRECTION_TIER_NONE":        1,
		"CORRECTION_TIER_PROSE":       2,
		"CORRECTION_TIER_FULL":        3,
	}
)

func (x CorrectionTier) Enum() *CorrectionTier {
	p := new(CorrectionTier)
	*p = x
	return p
}

func (x CorrectionTier) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (CorrectionTier) Descriptor() protoreflect.EnumDescriptor {
	return file_record_proto_enumTypes[0].Descriptor()
}

func (CorrectionTier) Type() protoreflect.EnumType {
	return &file_record_proto_enumTypes[0]
}

func (x CorrectionTier) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use CorrectionTier.Descriptor instead.
func (CorrectionTier) EnumDescriptor() ([]byte, []int) {
...
var (
	EventType_name = map[int32]string{
		...
		36: "EVENT_TYPE_CAST",
		37: "EVENT_TYPE_DISPATCH",
		38: "EVENT_TYPE_SITTING_LIMIT",
...
// ClassMaterial is where a gap's materiality starts: from its class. A claim, figure or citation
// defect changes what a reader concludes whatever it is graded, so its class is `always`; a
// template-shape defect changes no conclusion, so its class is `never`; every other class goes
// by the gap's current severity, medium and above. The registry row carries the class's default,
// a coined class carries the one its coiner chose, and the write path stamps it onto every Mint,
// so every reader of "is this gap material" reads the same field.
type ClassMaterial int32

const (
	ClassMaterial_CLASS_MATERIAL_UNSPECIFIED ClassMaterial = 0
	ClassMaterial_CLASS_MATERIAL_ALWAYS      ClassMaterial = 1
	ClassMaterial_CLASS_MATERIAL_NEVER       ClassMaterial = 2
	ClassMaterial_CLASS_MATERIAL_BY_GRADE    ClassMaterial = 3
)

// Enum value maps for ClassMaterial.
var (
	ClassMaterial_name = map[int32]string{
		0: "CLASS_MATERIAL_UNSPECIFIED",
		1: "CLASS_MATERIAL_ALWAYS",
		2: "CLASS_MATERIAL_NEVER",
		3: "CLASS_MATERIAL_BY_GRADE",
	}
	ClassMaterial_value = map[string]int32{
		"CLASS_MATERIAL_UNSPECIFIED": 0,
		"CLASS_MATERIAL_ALWAYS":      1,
		"CLASS_MATERIAL_NEVER":       2,
		"CLASS_MATERIAL_BY_GRADE":    3,
	}
)

func (x ClassMaterial) Enum() *ClassMaterial {
	p := new(ClassMaterial)
	*p = x
	return p
}

func (x ClassMaterial) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ClassMaterial) Descriptor() protoreflect.EnumDescriptor {
	return file_record_proto_enumTypes[6].Descriptor()
}

func (ClassMaterial) Type() protoreflect.EnumType {
	return &file_record_proto_enumTypes[6]
}

func (x ClassMaterial) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ClassMaterial.Descriptor instead.
func (ClassMaterial) EnumDescriptor() ([]byte, []int) {
...
// FindingAbout is WHAT A FINDING IS ANCHORED TO when the thing it names is not on the page.
//
// A finding used to require a live report quote, so a defect about an ABSENCE had to borrow an
// innocent sentence as a handle. Measured in research/2026-09-02_quadratic-formula: a missing line
// of inquiry was pinned to the Diophantus sentence the finding explicitly called fine, and a
// missing risk matrix — the word "risk" occurs zero times in that report — was pinned to the
// opening of section F. "A reader of the gap list will land on good prose and have to read three
// paragraphs to learn that the sentence is only a handle."
//
// IT THROTTLED THE STEELMAN DUTY IN PARTICULAR. Red's duty is to audit the REASONS declined and
// abandoned lines were set aside, and the strongest such findings are arguments against a reason —
// which is not report prose at all. The audit worked anyway, and every one of its findings
// mis-anchored.
//
...
// SourceTextRead is HOW MUCH OF THE SOURCE THIS SEAT ACTUALLY READ, and it is the blue-side twin
// of red's SourceOutcome. It exists because the distinction it carries was the load-bearing caveat
// of a whole run and lived only as a PROSE SUBSTRING: "unreachable from this container". The run
// then counted that phrase by grep and got it wrong twice — once counting a sentence that was
// counting itself — because a grep cannot tell a source DISCUSSED from one merely LISTED.
//
// The default is the honest one. A citation nobody has asserted a reading for is UNREAD, so the
// weak claim is what you get for free and the strong one has to be made on purpose.
type SourceTextRead int32

const (
	SourceTextRead_SOURCE_TEXT_READ_UNSPECIFIED  SourceTextRead = 0
	SourceTextRead_SOURCE_TEXT_READ_UNREAD       SourceTextRead = 1
	SourceTextRead_SOURCE_TEXT_READ_SUMMARY_ONLY SourceTextRead = 2
	SourceTextRead_SOURCE_TEXT_READ_LEAF         SourceTextRead = 3
)

// Enum value maps for SourceTextRead.
var (
	SourceTextRead_name = map[int32]string{
		0: "SOURCE_TEXT_READ_UNSPECIFIED",
		1: "SOURCE_TEXT_READ_UNREAD",
		2: "SOURCE_TEXT_READ_SUMMARY_ONLY",
		3: "SOURCE_TEXT_READ_LEAF",
	}
	SourceTextRead_value = map[string]int32{
		"SOURCE_TEXT_READ_UNSPECIFIED":  0,
		"SOURCE_TEXT_READ_UNREAD":       1,
		"SOURCE_TEXT_READ_SUMMARY_ONLY": 2,
		"SOURCE_TEXT_READ_LEAF":         3,
	}
)

func (x SourceTextRead) Enum() *SourceTextRead {
	p := new(SourceTextRead)
	*p = x
	return p
}

func (x SourceTextRead) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SourceTextRead) Descriptor() protoreflect.EnumDescriptor {
	return file_record_proto_enumTypes[17].Descriptor()
}

func (SourceTextRead) Type() protoreflect.EnumType {
	return &file_record_proto_enumTypes[17]
}

func (x SourceTextRead) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SourceTextRead.Descriptor instead.
func (SourceTextRead) EnumDescriptor() ([]byte, []int) {
	return file_record_proto_rawDescGZIP(), []int{17}
}

// SourceTextOrigin is WHERE THE CITED TEXT CAME FROM, and the tool stamps it; a seat never types
// it. It is the twin of SourceTextRead: that one is how much the seat read, this one is whether
// what it read is the author's text or the local engine's reading of page images. A bool
// `ocr_derived` would fold a scan nobody read and an author's own text into one `false`, and
// "false" on an unread scan claims the text is trustworthy.
type SourceTextOrigin int32

const (
	SourceTextOrigin_SOURCE_TEXT_ORIGIN_UNSPECIFIED  SourceTextOrigin = 0
	SourceTextOrigin_SOURCE_TEXT_ORIGIN_EMBEDDED     SourceTextOrigin = 1
	SourceTextOrigin_SOURCE_TEXT_ORIGIN_OCR          SourceTextOrigin = 2
	SourceTextOrigin_SOURCE_TEXT_ORIGIN_NONE         SourceTextOrigin = 3
	SourceTextOrigin_SOURCE_TEXT_ORIGIN_NOT_RECORDED SourceTextOrigin = 4
)

// Enum value maps for SourceTextOrigin.
var (
	SourceTextOrigin_name = map[int32]string{
		0: "SOURCE_TEXT_ORIGIN_UNSPECIFIED",
		1: "SOURCE_TEXT_ORIGIN_EMBEDDED",
		2: "SOURCE_TEXT_ORIGIN_OCR",
		3: "SOURCE_TEXT_ORIGIN_NONE",
		4: "SOURCE_TEXT_ORIGIN_NOT_RECORDED",
	}
	SourceTextOrigin_value = map[string]int32{
		"SOURCE_TEXT_ORIGIN_UNSPECIFIED":  0,
		"SOURCE_TEXT_ORIGIN_EMBEDDED":     1,
		"SOURCE_TEXT_ORIGIN_OCR":          2,
		"SOURCE_TEXT_ORIGIN_NONE":         3,
		"SOURCE_TEXT_ORIGIN_NOT_RECORDED": 4,
	}
)

func (x SourceTextOrigin) Enum() *SourceTextOrigin {
	p := new(SourceTextOrigin)
	*p = x
	return p
}

func (x SourceTextOrigin) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SourceTextOrigin) Descriptor() protoreflect.EnumDescriptor {
	return file_record_proto_enumTypes[18].Descriptor()
}

func (SourceTextOrigin) Type() protoreflect.EnumType {
	return &file_record_proto_enumTypes[18]
}

func (x SourceTextOrigin) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SourceTextOrigin.Descriptor instead.
func (SourceTextOrigin) EnumDescriptor() ([]byte, []int) {
...
// tool-emitted type never breaks that filter.
//
// The zero of each is declared because proto3 requires one, and is UNWRITABLE: schema.go skips the
// UNSPECIFIED zero when building the enum vocabulary table ("absence is NULL here"), so the table
// holds only real values and the column's FOREIGN KEY into it refuses an explicit zero. With
// `required: true` the column is NOT NULL as well: NULL refused by the requirement, the zero by
// the key.
type LogSource int32

const (
	LogSource_LOG_SOURCE_UNSPECIFIED LogSource = 0
	LogSource_LOG_SOURCE_SEAT        LogSource = 1
	LogSource_LOG_SOURCE_TOOL        LogSource = 2
)
...
// the wording. estoppel.go:142-154 carries the history: the prose is aimed at a seat and must stay
// editable, so the count an operator reads must not move when it is edited (#283). A `{text}`-only
// message would have reverted that fix silently.
type LogType int32
...
func (x *Event) GetSittingOpen() *SittingOpen {
	if x != nil {
		if x, ok := x.Body.(*Event_SittingOpen); ok {
			return x.SittingOpen
		}
	}
	return nil
}

func (x *Event) GetSittingClose() *SittingClose {
	if x != nil {
		if x, ok := x.Body.(*Event_SittingClose); ok {
			return x.SittingClose
		}
	}
	return nil
}
...
func (x *Event) GetDispatch() *Dispatch {
	if x != nil {
		if x, ok := x.Body.(*Event_Dispatch); ok {
			return x.Dispatch
		}
	}
	return nil
...
func (x *Event) GetSittingLimit() *SittingLimit {
	if x != nil {
		if x, ok := x.Body.(*Event_SittingLimit); ok {
			return x.SittingLimit
		}
	}
	return nil
}

func (x *Event) GetCorrection() *Correction {
	if x != nil {
		if x, ok := x.Body.(*Event_Correction); ok {
			return x.Correction
		}
	}
	return nil
...
type Event_SittingOpen struct {
	SittingOpen *SittingOpen `protobuf:"bytes,53,opt,name=sitting_open,json=sittingOpen,proto3,oneof"`
}

type Event_SittingClose struct {
	SittingClose *SittingClose `protobuf:"bytes,54,opt,name=sitting_close,json=sittingClose,proto3,oneof"`
}

type Event_Cast struct {
	Cast *Cast `protobuf:"bytes,55,opt,name=cast,proto3,oneof"`
}

type Event_Dispatch struct {
	Dispatch *Dispatch `protobuf:"bytes,56,opt,name=dispatch,proto3,oneof"`
}

type Event_SittingLimit struct {
	SittingLimit *SittingLimit `protobuf:"bytes,57,opt,name=sitting_limit,json=sittingLimit,proto3,oneof"`
}

type Event_Correction struct {
	Correction *Correction `protobuf:"bytes,58,opt,name=correction,proto3,oneof"`
}
...
func (*Event_SittingOpen) isEvent_Body() {}

func (*Event_SittingClose) isEvent_Body() {}

func (*Event_Cast) isEvent_Body() {}

func (*Event_Dispatch) isEvent_Body() {}

func (*Event_SittingLimit) isEvent_Body() {}

func (*Event_Correction) isEvent_Body() {}
...
type Mint struct {
	...
	// §III.B.2): wherever the class goes by grade, materiality reads MASS[current severity], so an
	// absent severity reads as mass zero — not material, readying nobody and never holding the
	// gate — which is exactly "trivial". The release sweep found it: gaps minted without one ended
	// runs VERIFIED over open work. Cost is still reported, not read.
	...
	// WHERE THIS GAP'S MATERIALITY STARTS, stamped by the write path from the class — the staged
	// registry's row, or the run's own `class new` — and never by a seat: a lens that could set it
	// could make any gap it minted immaterial. Required, because an absent value would leave every
	// reader of "is this gap material" to guess, and the guess would be the grade alone.
	ClassMaterial *ClassMaterial `protobuf:"varint,25,opt,name=class_material,json=classMaterial,proto3,enum=feov.record.v1.ClassMaterial,oneof" json:"class_material,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Mint) GetClassMaterial() ClassMaterial {
	if x != nil && x.ClassMaterial != nil {
		return *x.ClassMaterial
	}
	return ClassMaterial_CLASS_MATERIAL_UNSPECIFIED
}
...
type ClassNew struct {
	...
	// The coined class's materiality default. The verb always writes it (`by_grade` unless the
	// coiner says otherwise), so a class the run coined carries its default on the record, where
	// the Mint stamp and the capture's adoption text read it.
	MaterialDefault *ClassMaterial `protobuf:"varint,5,opt,name=material_default,json=materialDefault,proto3,enum=feov.record.v1.ClassMaterial,oneof" json:"material_default,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}
...
func (x *ClassNew) GetMaterialDefault() ClassMaterial {
	if x != nil && x.MaterialDefault != nil {
		return *x.MaterialDefault
	}
	return ClassMaterial_CLASS_MATERIAL_UNSPECIFIED
}
...
type Close struct {
	...
	// (feov.record.v1.prose) is spelled out in this message and in Outcome because each has a FIELD
	// named `prose`, which the bare option name would resolve to.
...
type Cite struct {
	...
	// WHERE THE QUOTE SITS, stamped by the tool; a same-sitting correction carries all five
	// unchanged. Not `(sql).required`: that annotation teaches a flag, and no seat types these —
	// validate refuses an unstamped origin instead.
	//
	// ocr_quote is the seat's span, verbatim as typed; pages are the PDF page indexes the binary
	// found it on, ascending and unique; ocr_engine and ocr_text_sha name WHICH reading located
	// them, because run-archive keeps no cache and the event is the only thing that outlives it.
	// A page is bound to that page's pixels, not to the engine: a later engine can stop finding
	// the span, never move it.
	SourceTextOrigin *SourceTextOrigin `protobuf:"varint,11,opt,name=source_text_origin,json=sourceTextOrigin,proto3,enum=feov.record.v1.SourceTextOrigin,oneof" json:"source_text_origin,omitempty"`
	OcrQuote         *string           `protobuf:"bytes,12,opt,name=ocr_quote,json=ocrQuote,proto3,oneof" json:"ocr_quote,omitempty"`
...
func (x *Cite) GetSourceTextRead() SourceTextRead {
	if x != nil && x.SourceTextRead != nil {
		return *x.SourceTextRead
	}
	return SourceTextRead_SOURCE_TEXT_READ_UNSPECIFIED
}

func (x *Cite) GetSourceTextOrigin() SourceTextOrigin {
	if x != nil && x.SourceTextOrigin != nil {
		return *x.SourceTextOrigin
	}
	return SourceTextOrigin_SOURCE_TEXT_ORIGIN_UNSPECIFIED
}

func (x *Cite) GetOcrQuote() string {
	if x != nil && x.OcrQuote != nil {
		return *x.OcrQuote
	}
	return ""
}

func (x *Cite) GetPages() []int32 {
	if x != nil {
		return x.Pages
	}
	return nil
...
func (x *Cite) GetOcrEngine() string {
...
func (x *Cite) GetOcrTextSha() string {
	if x != nil && x.OcrTextSha != nil {
		return *x.OcrTextSha
	}
	return ""
}
...
type Verify struct {
	...
	// THE PAGE IMAGE RED CHECKED, for a citation whose OCR quote carries pages. page is the one red
	// names; page_render_sha is the sha256 of the image on disk when the verification was written,
	// and reading_render_sha the render the reading was made from. A mismatch between the two is
	// recorded, not refused: the image still shows the page, and two fields let any reader compare
	// them. All three are set together or not at all.
...
func (x *Verify) GetPageRenderSha() string {
	if x != nil && x.PageRenderSha != nil {
		return *x.PageRenderSha
	}
	return ""
}

func (x *Verify) GetReadingRenderSha() string {
	if x != nil && x.ReadingRenderSha != nil {
...
type Log struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Text  *string                `protobuf:"bytes,1,opt,name=text,proto3,oneof" json:"text,omitempty"`
	Type  *LogType               `protobuf:"varint,2,opt,name=type,proto3,enum=feov.record.v1.LogType,oneof" json:"type,omitempty"`
	// SOURCE IS SET BY THE WRITE PATH, NOT ASKED. It is deliberately NOT `required`: that
	// annotation means "a flag the seat supplies", and vocabulary_test enforces exactly that
	// mapping. A seat cannot honestly claim to be the tool, so the seat verb sets SEAT and the
	// tool's own writes set TOOL, at every call site.
	Source *LogSource `protobuf:"varint,4,opt,name=source,proto3,enum=feov.record.v1.LogSource,oneof" json:"source,omitempty"`
...
func (x *Log) GetEstoppedBy() string {
...
func (x *SittingOpen) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *SittingClose) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
// SittingLimit is a seat's sitting stopped at the run's per-sitting tool-call limit (the run term
// maxSittingCalls in inputs/run-config.json). The PreToolUse hook counts each seat's calls from its
// register and refuses every call past the limit; the first refusal hands the sitting to
// feov-sitting-write, which writes this under seat `harness`.
//
...
type SittingLimit struct {
	...
	AgentType     *string                `protobuf:"bytes,2,opt,name=agent_type,json=agentType,proto3,oneof" json:"agent_type,omitempty"`
	SeatId        *string                `protobuf:"bytes,3,opt,name=seat_id,json=seatId,proto3,oneof" json:"seat_id,omitempty"`
	Sitting       *int32                 `protobuf:"varint,4,opt,name=sitting,proto3,oneof" json:"sitting,omitempty"`
	Limit         *int32                 `protobuf:"varint,5,opt,name=limit,proto3,oneof" json:"limit,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SittingLimit) Reset() {
	*x = SittingLimit{}
...
func (x *SittingLimit) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SittingLimit) ProtoMessage() {}

func (x *SittingLimit) ProtoReflect() protoreflect.Message {
	mi := &file_record_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use SittingLimit.ProtoReflect.Descriptor instead.
func (*SittingLimit) Descriptor() ([]byte, []int) {
	return file_record_proto_rawDescGZIP(), []int{40}
}

func (x *SittingLimit) GetAgentId() string {
	if x != nil && x.AgentId != nil {
		return *x.AgentId
	}
	return ""
...
func (x *SittingLimit) GetAgentType() string {
	if x != nil && x.AgentType != nil {
		return *x.AgentType
	}
	return ""
...
func (x *SittingLimit) GetSeatId() string {
	if x != nil && x.SeatId != nil {
		return *x.SeatId
	}
	return ""
...
func (x *SittingLimit) GetSitting() int32 {
	if x != nil && x.Sitting != nil {
		return *x.Sitting
	}
	return 0
}

func (x *SittingLimit) GetLimit() int32 {
...
// Cast is the run's admissible seats (plans/roundless.md §III.B.1). setup writes it ONCE, under
// seat `harness`, before any seat registers: the areas selected, red-chair, blue-lane-1..N,
// blue-synthesize, blue-respond, frontier, judge, judge-terminal, assemble, and judge-petition-<s>
...
// Dispatch is the chair engaging ONE party: the seat, the gaps it is engaged on, and the report
// head it is to audit (plans/roundless.md §III.B.1). The chair's `dispatch next` writes one of
// these per ready party in one sitting, so "dispatch D" — the unit the parity audit and the
// exchange counter reason about — is the set written in one chair sitting, read off events_w.
//
...
func (x *Dispatch) GetSeatId() string {
	if x != nil && x.SeatId != nil {
		return *x.SeatId
	}
	return ""
...
// Correction is a seat correcting its OWN act within the sitting that wrote it.
//
// The record stays append-only: the corrected act is not edited or removed. The seat re-runs the
// act's own verb with the text it meant and `--corrects <key>`, and the tool writes the replacement
// and this event in one transaction. Every reader then shows the corrected act STRUCK, beside the
// replacement that takes its place — never hidden. What may change is the type's CorrectionTier;
// who and when is fixed: the writing seat, the same sitting, and only while no other seat has acted
// since.
type Correction struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// corrects is the key of the act this strikes. No foreign key: `events.key` is a PARTIAL unique
	// index, which SQLite does not accept as a parent key, so the write checks the target exists
	// inside the transaction that inserts this row.
	Corrects *string `protobuf:"bytes,1,opt,name=corrects,proto3,oneof" json:"corrects,omitempty"`
	// replacement is the key of the act that takes the corrected one's place.
	Replacement   *string `protobuf:"bytes,2,opt,name=replacement,proto3,oneof" json:"replacement,omitempty"`
	Why           *string `protobuf:"bytes,3,opt,name=why,proto3,oneof" json:"why,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Correction) GetCorrects() string {
...
func (x *Correction) GetReplacement() string {
...
func (x *Correction) GetWhy() string {
	if x != nil && x.Why != nil {
		return *x.Why
	}
	return ""
...
var file_record_proto_extTypes = []protoimpl.ExtensionInfo{
	...
	},
	{
		...
		Field:         50008,
		Name:          "feov.record.v1.prose",
		Tag:           "varint,50008,opt,name=prose",
		Filename:      "record.proto",
	},
	{
	...
	{
		ExtendedType:  (*descriptorpb.EnumValueOptions)(nil),
		ExtensionType: (*CorrectionTier)(nil),
		Field:         50007,
		Name:          "feov.record.v1.correct",
		Tag:           "varint,50007,opt,name=correct,enum=feov.record.v1.CorrectionTier",
		Filename:      "record.proto",
	},
	{
...
var (
	...
	// prose IS WHETHER A SAME-SITTING CORRECTION MAY CHANGE THIS FIELD when its event type's tier is
	// PROSE (see CorrectionTier). It is DECLARED on every string field a free-text flag fills, true
	// or false, so "nobody asked" cannot pass for either answer: `true` is the seat's own wording,
	// which a correction exists to fix; `false` marks free text the tier freezes anyway (a closure's
	// verification tool is part of the anchor triple, not of its argument). A test walks every
	// free-text flag of every correctable verb to the field it fills and refuses one that does not
	// declare this.
	//
	// optional bool prose = 50008;
	E_Prose = &file_record_proto_extTypes[1]
)
...
var (
	...
	// values and not others. The rule the answers follow: an act that CREATES an identity (a gap, a
	// motion, a finding, a citation, a class) is never correctable; an act that REFERENCES one may
	// be.
	//
...
const file_record_proto_rawDesc = "" +
	...
	"\rsitting_close\x186 \x01(\v2\x1c.feov.record.v1.SittingCloseH\x00R\fsittingClose\x12*\n" +
	"\x04cast\x187 \x01(\v2\x14.feov.record.v1.CastH\x00R\x04cast\x126\n" +
	"\bdispatch\x188 \x01(\v2\x18.feov.record.v1.DispatchH\x00R\bdispatch\x12C\n" +
	"\rsitting_limit\x189 \x01(\v2\x1c.feov.record.v1.SittingLimitH\x00R\fsittingLimit\x12<\n" +
	"\n" +
	"correction\x18: \x01(\v2\x1a.feov.record.v1.CorrectionH\x00R\n" +
	"correctionB\x06\n" +
	"\x04bodyB\x05\n" +
	"\x03_tsB\n" +
	"\n" +
	...
	"J\x04\b;\x10QJ\x04\b$\x10%R\x05roundR\x0eschema_versionR\aopinion\"\xf0\x04\n" +
	...
	"\x05class\x18\x03 \x01(\tB\xd5\x01\x82\xb5\x18\xd0\x01\b\x01\x1a\xcb\x01the slug — one the registry has, or one you coined first with `class new`. A gap with no class cannot be counted with its kind, and the class report is how a run learns it keeps making the same mistakeH\x02R\x05class\x88\x01\x01\x12 \n" +
	...
	"\bseverity\x18\x10 \x01(\x0e2\x15.feov.record.v1.GradeB\xc4\x01\x82\xb5\x18\xbf\x01\b\x01\x1a\xba\x01materiality reads it wherever the class goes by grade (medium and above holds the gate), so an absent grade would score as TRIVIAL and the gap would read as harmless rather than ungradedH\x10R\bseverity\x88\x01\x01\x12\xbf\x01\n" +
	"\n" +
	...
	"\f_mint_reasonB\x11\n" +
	"\x0f_class_materialJ\x04\b\v\x10\fR\afix_old\"\xaf\x02\n" +
	"\bClassNew\x12\x17\n" +
	...
	"\rdistinguisher\x18\x04 \x01(\tH\x03R\rdistinguisher\x88\x01\x01\x12M\n" +
	"\x10material_default\x18\x05 \x01(\x0e2\x1d.feov.record.v1.ClassMaterialH\x04R\x0fmaterialDefault\x88\x01\x01B\a\n" +
	"\x05_slugB\r\n" +
	"\v_definitionB\v\n" +
	"\t_neighborB\x10\n" +
	"\x0e_distinguisherB\x13\n" +
	"\x11_material_default\"\xfa\b\n" +
	"\x05Close\x12J\n" +
	...
	"\x06_prose\"\xcd\x01\n" +
	"\aClosing\x12-\n" +
	"\x06gap_id\x18\x01 \x01(\tB\x11\x82\xb5\x18\r\"\vmint.gap_idH\x00R\x05gapId\x88\x01\x01\x12\x7f\n" +
	"\x04text\x18\x02 \x01(\tBf\x82\xb5\x18^\b\x01\x12\x06reason\x1aRthe closing argument for this gap — the report renders it under the gap's docket\xc0\xb5\x18\x01H\x01R\x04text\x88\x01\x01B\t\n" +
	"\a_gap_idB\a\n" +
	"\x05_text\"\xd0\x03\n" +
	"\aRegrade\x12-\n" +
	...
	"\x0fcomplexity_cost\x18\x05 \x01(\x0e2\x15.feov.record.v1.GradeH\x04R\x0ecomplexityCost\x88\x01\x01\x12Y\n" +
	"\x05basis\x18\x06 \x01(\tB>\x82\xb5\x186\b\x01\x12\x06reason\x1a*grade movement is recorded with its reason\xc0\xb5\x18\x01H\x05R\x05basis\x88\x01\x01B\t\n" +
	"\a_gap_idB\v\n" +
	"\t_severityB\r\n" +
	"\v_likelihoodB\t\n" +
	"\a_impactB\x12\n" +
	...
	"\x05pages\x18\r \x03(\x05B\x04\xc0\xb5\x18\x00R\x05pages\x12(\n" +
	"\n" +
	...
	"\t_cite_keyB\a\n" +
	"\x05_textB\x13\n" +
	"\x11_source_text_readB\x15\n" +
	"\x13_source_text_originB\f\n" +
	"\n" +
	"_ocr_quoteB\r\n" +
	"\v_ocr_engineB\x0f\n" +
	...
	"\x05label\x18\v \x01(\tH\tR\x05label\x88\x01\x01\x12\x17\n" +
	...
	"\x0fobserved_output\x18\x05 \x01(\tH\x04R\x0eobservedOutput\x88\x01\x01\x12)\n" +
	"\x04note\x18\x06 \x01(\tB\x10\x82\xb5\x18\b\x12\x06reason\xc0\xb5\x18\x01H\x05R\x04note\x88\x01\x01B\f\n" +
	"\n" +
	"_proof_shaB\r\n" +
	"\v_reproducedB\f\n" +
	"\n" +
	...
	"\v_exact_span\">\n" +
	"\bRevision\x12)\n" +
	"\x04text\x18\x01 \x01(\tB\x10\x82\xb5\x18\b\x12\x06reason\xc0\xb5\x18\x01H\x00R\x04text\x88\x01\x01B\a\n" +
	"\x05_text\"\xbc\x03\n" +
	"\x06Retire\x12|\n" +
	...
	"\x03row\x18\x02 \x01(\tB\x10\x82\xb5\x18\b\x12\x06reason\xc0\xb5\x18\x01H\x01R\x03row\x88\x01\x01B\t\n" +
	"\a_gap_idB\x06\n" +
	"\x04_row\"\xb4\x03\n" +
	"\x03Log\x12)\n" +
	"\x04text\x18\x01 \x01(\tB\x10\x82\xb5\x18\b\x12\x06reason\xc0\xb5\x18\x01H\x00R\x04text\x88\x01\x01\x12\xf6\x01\n" +
	"\x04type\x18\x02 \x01(\x0e2\x17.feov.record.v1.LogTypeB\xc3\x01\x82\xb5\x18\xbe\x01\b\x01\x1a\xb9\x01the operator triages this channel by FILTERING on the type; an untyped entry hands the reader back the reading this field exists to replace — measured at 142,891 characters on one runH\x01R\x04type\x88\x01\x01\x126\n" +
	"\x06source\x18\x04 \x01(\x0e2\x19.feov.record.v1.LogSourceH\x02R\x06source\x88\x01\x01\x12$\n" +
	"\vestopped_by\x18\x03 \x01(\tH\x03R\n" +
	...
	"\asubject\x18\x02 \x01(\x0e2\x1d.feov.record.v1.MotionSubjectH\x02R\asubject\x88\x01\x01\x12\xaa\x01\n" +
	...
	"\asubject\x18\x02 \x01(\x0e2\x1d.feov.record.v1.MotionSubjectH\x01R\asubject\x88\x01\x01\x12!\n" +
	"\x06reason\x18\x03 \x01(\tB\x04\xc0\xb5\x18\x01H\x02R\x06reason\x88\x01\x01B\f\n" +
	"\n" +
	"_motion_idB\n" +
	"\n" +
	"\b_subjectB\t\n" +
	...
	"\aseat_id\x18\x03 \x01(\tH\x02R\x06seatId\x88\x01\x01\x12\x1d\n" +
	"\asitting\x18\x04 \x01(\x05H\x03R\asitting\x88\x01\x01\x12\x19\n" +
	"\x05limit\x18\x05 \x01(\x05H\x04R\x05limit\x88\x01\x01B\v\n" +
	"\t_agent_idB\r\n" +
	"\v_agent_typeB\n" +
	"\n" +
	"\b_seat_idB\n" +
	...
	"\averdict\x18\x01 \x01(\x0e2\x1a.feov.record.v1.RunOutcomeB\xb5\x01\x82\xb5\x18\xb0\x01\b\x01\x12\x02as\x1a\xa7\x01the run's terminal verdict — an outcome event without one records that the run ENDED and not how, and every reader downstream sees a run that never reached a verdictH\x00R\averdict\x88\x01\x01\x12\xfe\x01\n" +
	"\x05prose\x18\x02 \x01(\tB\xe2\x01\x82\xb5\x18\xd9\x01\b\x01\x12\x06reason\x1a\xcc\x01how this run ended, in your words — the verdict is derived from the record, but your account of the sitting is not, and on an UNVERIFIED run it is the only evidence of why the run stopped will ever have\xc0\xb5\x18\x01H\x01R\x05prose\x88\x01\x01\x12$\n" +
	...
	"\x05_text\"\xad\x01\n" +
	"\x04Halt\x12\x98\x01\n" +
	"\aopinion\x18\x01 \x01(\tBy\x82\xb5\x18q\b\x01\x12\x06reason\x1aethe written opinion capture relays verbatim — a halt nobody can read is a stop with no stated cause\xc0\xb5\x18\x01H\x00R\aopinion\x88\x01\x01B\n" +
	"\n" +
	"\b_opinion\"\xcc\x01\n" +
	"\aCertify\x12\xb2\x01\n" +
	"\tstatement\x18\x01 \x01(\tB\x8e\x01\x82\xb5\x18\x85\x01\b\x01\x12\x06reason\x1aywhat you would want a human to re-examine — the bench keeps no memory between runs, so this statement is its continuity\xc0\xb5\x18\x01H\x00R\tstatement\x88\x01\x01B\f\n" +
	"\n" +
	"_statement\"F\n" +
	"\aDeclare\x12/\n" +
	"\aholding\x18\x01 \x01(\tB\x10\x82\xb5\x18\b\x12\x06reason\xc0\xb5\x18\x01H\x00R\aholding\x88\x01\x01B\n" +
	"\n" +
	"\b_holding\"\xed\x02\n" +
	"\n" +
	"Correction\x12S\n" +
	"\bcorrects\x18\x01 \x01(\tB2\x82\xb5\x18.\b\x01\x1a*the key of the act this correction strikesH\x00R\bcorrects\x88\x01\x01\x12^\n" +
	"\vreplacement\x18\x02 \x01(\tB7\x82\xb5\x183\b\x01\x1a/the key of the act that replaces the struck oneH\x01R\vreplacement\x88\x01\x01\x12\x84\x01\n" +
	"\x03why\x18\x03 \x01(\tBm\x82\xb5\x18e\b\x01\x12\x0ecorrection-why\x1aQwhat was wrong with the corrected act — a reader sees it beside the struck text\xc0\xb5\x18\x01H\x02R\x03why\x88\x01\x01B\v\n" +
	"\t_correctsB\x0e\n" +
	"\f_replacementB\x06\n" +
	"\x04_why*\xdf\x03\n" +
	"\x0eCorrectionTier\x12\x1f\n" +
	...
	"\x11EVENT_TYPE_MOTION\x10\x11\x1a[\x8a\xb5\x18Sa motion filed: a grade contested, a petition to the bench, or a direction proposed\xb8\xb5\x18\x01\x12T\n" +
	...
	"\x13EVENT_TYPE_DISPATCH\x10%\x1a\xa9\x01\x8a\xb5\x18\xa0\x01the chair engaging one party — a seat and the gaps it is engaged on — pinned to the report head it audits; the parties of one chair sitting are one dispatch\xb8\xb5\x18\x01\x12\xd1\x01\n" +
	"\x18EVENT_TYPE_SITTING_LIMIT\x10&\x1a\xb2\x01\x8a\xb5\x18\xa9\x01a seat's sitting stopped at the run's per-sitting tool-call limit — the hook refuses every further call in it, and this records which seat, which sitting and the limit\xb8\xb5\x18\x01\x12\xcd\x01\n" +
	"\x15EVENT_TYPE_CORRECTION\x10'\x1a\xb1\x01\x8a\xb5\x18\xa8\x01a seat correcting its own act within the sitting that wrote it: names the act it strikes, the replacement that takes its place, and why — both acts stay on the record\xb8\xb5\x18\x01\"\x04\b\x15\x10\x15*\x12EVENT_TYPE_OPINION*\x9e\x06\n" +
	"\x05Grade\x12\"\n" +
	...
	"\tGRADE_LOW\x10\x02\x1a\x14\x8a\xb5\x18\x05minor\xa9\xb5\x18\x00\x00\x00\x00\x00\x00\xf0?\x12F\n" +
	"\x10GRADE_LOW_MEDIUM\x10\x03\x1a0\x8a\xb5\x18!below the by-grade material floor\xa9\xb5\x18\x00\x00\x00\x00\x00\x00\xf8?\x12<\n" +
	"\fGRADE_MEDIUM\x10\x04\x1a*\x8a\xb5\x18\x1bthe by-grade material floor\xa9\xb5\x18\x00\x00\x00\x00\x00\x00\x00@\x12V\n" +
	"\x11GRADE_MEDIUM_HIGH\x10\x05\x1a?\x8a\xb5\x180above the by-grade material floor, below serious\xa9\xb5\x18\x00\x00\x00\x00\x00\x00\x04@\x12&\n" +
	"\n" +
	...
	"\x11CHECK_KIND_SOURCE\x10\x03\x1ar\x8a\xb5\x18nverifying an external source settles it — the claim stands or falls on what the cited material actually says*\xba\x03\n" +
	"\rClassMaterial\x12\x1e\n" +
	...
	"\x17CLASS_MATERIAL_BY_GRADE\x10\x03\x1aP\x8a\xb5\x18La gap of this class is material when its current severity is medium or above*\xce\n" +
	"\n" +
	...
	"\x1dSOURCE_TEXT_READ_SUMMARY_ONLY\x10\x02\x1a\xd2\x01\x8a\xb5\x18\xcd\x01read only through someone else's account of it — an abstract, a secondary description, or the summary of an INTERESTED party. Everything the report says about its contents is that account, not the source\x12\xa9\x01\n" +
	"\x15SOURCE_TEXT_READ_LEAF\x10\x03\x1a\x8d\x01\x8a\xb5\x18\x88\x01the source's own text was read at the leaf, in the bytes the run cached. The only value that licenses a claim about what the source SAYS*\xb3\x04\n" +
	"\x10SourceTextOrigin\x12\"\n" +
	"\x1eSOURCE_TEXT_ORIGIN_UNSPECIFIED\x10\x00\x12v\n" +
	...
	"\x04mass\x12!.google.protobuf.EnumValueOptions\x18Ն\x03 \x01(\x01R\x04mass\x88\x01\x01:`\n" +
	"\acorrect\x12!.google.protobuf.EnumValueOptions\x18׆\x03 \x01(\x0e2\x1e.feov.record.v1.CorrectionTierR\acorrect\x88\x01\x01:Q\n" +
...
var file_record_proto_enumTypes = make([]protoimpl.EnumInfo, 24)
...
var file_record_proto_goTypes = []any{
	...
	(ClassMaterial)(0),                    // 6: feov.record.v1.ClassMaterial
	(Disposition)(0),                      // 7: feov.record.v1.Disposition
	(SourceOutcome)(0),                    // 8: feov.record.v1.SourceOutcome
	(Confidence)(0),                       // 9: feov.record.v1.Confidence
	(Soundness)(0),                        // 10: feov.record.v1.Soundness
	(AvenueStatus)(0),                     // 11: feov.record.v1.AvenueStatus
	...
	(*SqlCheck)(nil),                      // 24: feov.record.v1.SqlCheck
	(*Sql)(nil),                           // 25: feov.record.v1.Sql
	(*Event)(nil),                         // 26: feov.record.v1.Event
	(*TelemetryLine)(nil),                 // 27: feov.record.v1.TelemetryLine
	(*NewMint)(nil),                       // 28: feov.record.v1.NewMint
	(*SeverityTally)(nil),                 // 29: feov.record.v1.SeverityTally
	(*RepairRegression)(nil),              // 30: feov.record.v1.RepairRegression
	(*EdgeDeltas)(nil),                    // 31: feov.record.v1.EdgeDeltas
	(*Mint)(nil),                          // 32: feov.record.v1.Mint
	(*ClassNew)(nil),                      // 33: feov.record.v1.ClassNew
	(*Close)(nil),                         // 34: feov.record.v1.Close
	(*Closing)(nil),                       // 35: feov.record.v1.Closing
	(*Regrade)(nil),                       // 36: feov.record.v1.Regrade
	(*SpotCheck)(nil),                     // 37: feov.record.v1.SpotCheck
	(*Finding)(nil),                       // 38: feov.record.v1.Finding
	(*Observe)(nil),                       // 39: feov.record.v1.Observe
	(*Anchor)(nil),                        // 40: feov.record.v1.Anchor
	(*Cite)(nil),                          // 41: feov.record.v1.Cite
	(*Verify)(nil),                        // 42: feov.record.v1.Verify
	(*Proof)(nil),                         // 43: feov.record.v1.Proof
	(*Reproduce)(nil),                     // 44: feov.record.v1.Reproduce
	(*InquiryReview)(nil),                 // 45: feov.record.v1.InquiryReview
	(*Avenue)(nil),                        // 46: feov.record.v1.Avenue
	(*BaseIngest)(nil),                    // 47: feov.record.v1.BaseIngest
	(*BlueEdit)(nil),                      // 48: feov.record.v1.BlueEdit
	(*Revision)(nil),                      // 49: feov.record.v1.Revision
	(*Retire)(nil),                        // 50: feov.record.v1.Retire
	(*ManifestRow)(nil),                   // 51: feov.record.v1.ManifestRow
	(*Log)(nil),                           // 52: feov.record.v1.Log
	(*Motion)(nil),                        // 53: feov.record.v1.Motion
	(*GradeMotion)(nil),                   // 54: feov.record.v1.GradeMotion
	(*PetitionMotion)(nil),                // 55: feov.record.v1.PetitionMotion
	(*DocketMotion)(nil),                  // 56: feov.record.v1.DocketMotion
	(*DocketRuling)(nil),                  // 57: feov.record.v1.DocketRuling
	(*DirectionMotion)(nil),               // 58: feov.record.v1.DirectionMotion
	(*MotionRule)(nil),                    // 59: feov.record.v1.MotionRule
	(*MotionAppeal)(nil),                  // 60: feov.record.v1.MotionAppeal
	(*Register)(nil),                      // 61: feov.record.v1.Register
	(*SittingOpen)(nil),                   // 62: feov.record.v1.SittingOpen
	(*SittingClose)(nil),                  // 63: feov.record.v1.SittingClose
	(*SittingLimit)(nil),                  // 64: feov.record.v1.SittingLimit
	(*Cast)(nil),                          // 65: feov.record.v1.Cast
	(*Dispatch)(nil),                      // 66: feov.record.v1.Dispatch
	(*Gate)(nil),                          // 67: feov.record.v1.Gate
	(*Outcome)(nil),                       // 68: feov.record.v1.Outcome
	(*Position)(nil),                      // 69: feov.record.v1.Position
	(*Halt)(nil),                          // 70: feov.record.v1.Halt
	(*Certify)(nil),                       // 71: feov.record.v1.Certify
	(*Declare)(nil),                       // 72: feov.record.v1.Declare
	(*Correction)(nil),                    // 73: feov.record.v1.Correction
	nil,                                   // 74: feov.record.v1.NewMint.ByClassEntry
	(*descriptorpb.FieldOptions)(nil),     // 75: google.protobuf.FieldOptions
	(*descriptorpb.EnumValueOptions)(nil), // 76: google.protobuf.EnumValueOptions
	(*descriptorpb.MessageOptions)(nil),   // 77: google.protobuf.MessageOptions
}
var file_record_proto_depIdxs = []int32{
	1,   // 0: feov.record.v1.Event.type:type_name -> feov.record.v1.EventType
	61,  // 1: feov.record.v1.Event.register:type_name -> feov.record.v1.Register
	67,  // 2: feov.record.v1.Event.verdict:type_name -> feov.record.v1.Gate
	68,  // 3: feov.record.v1.Event.outcome:type_name -> feov.record.v1.Outcome
	69,  // 4: feov.record.v1.Event.position:type_name -> feov.record.v1.Position
	70,  // 5: feov.record.v1.Event.halt:type_name -> feov.record.v1.Halt
	71,  // 6: feov.record.v1.Event.certify:type_name -> feov.record.v1.Certify
	72,  // 7: feov.record.v1.Event.declare:type_name -> feov.record.v1.Declare
	53,  // 8: feov.record.v1.Event.motion:type_name -> feov.record.v1.Motion
	59,  // 9: feov.record.v1.Event.motion_rule:type_name -> feov.record.v1.MotionRule
	60,  // 10: feov.record.v1.Event.motion_appeal:type_name -> feov.record.v1.MotionAppeal
	32,  // 11: feov.record.v1.Event.mint:type_name -> feov.record.v1.Mint
	33,  // 12: feov.record.v1.Event.class_new:type_name -> feov.record.v1.ClassNew
	34,  // 13: feov.record.v1.Event.close:type_name -> feov.record.v1.Close
	35,  // 14: feov.record.v1.Event.closing:type_name -> feov.record.v1.Closing
	36,  // 15: feov.record.v1.Event.regrade:type_name -> feov.record.v1.Regrade
	37,  // 16: feov.record.v1.Event.spot_check:type_name -> feov.record.v1.SpotCheck
	38,  // 17: feov.record.v1.Event.finding:type_name -> feov.record.v1.Finding
	39,  // 18: feov.record.v1.Event.observe:type_name -> feov.record.v1.Observe
	40,  // 19: feov.record.v1.Event.anchor:type_name -> feov.record.v1.Anchor
	41,  // 20: feov.record.v1.Event.cite:type_name -> feov.record.v1.Cite
	42,  // 21: feov.record.v1.Event.verify:type_name -> feov.record.v1.Verify
	43,  // 22: feov.record.v1.Event.proof:type_name -> feov.record.v1.Proof
	44,  // 23: feov.record.v1.Event.reproduce:type_name -> feov.record.v1.Reproduce
	46,  // 24: feov.record.v1.Event.avenue:type_name -> feov.record.v1.Avenue
	48,  // 25: feov.record.v1.Event.blue_edit:type_name -> feov.record.v1.BlueEdit
	49,  // 26: feov.record.v1.Event.revision:type_name -> feov.record.v1.Revision
	50,  // 27: feov.record.v1.Event.retire:type_name -> feov.record.v1.Retire
	51,  // 28: feov.record.v1.Event.manifest_row:type_name -> feov.record.v1.ManifestRow
	52,  // 29: feov.record.v1.Event.log:type_name -> feov.record.v1.Log
	45,  // 30: feov.record.v1.Event.inquiry_review:type_name -> feov.record.v1.InquiryReview
	47,  // 31: feov.record.v1.Event.base_ingest:type_name -> feov.record.v1.BaseIngest
	62,  // 32: feov.record.v1.Event.sitting_open:type_name -> feov.record.v1.SittingOpen
	63,  // 33: feov.record.v1.Event.sitting_close:type_name -> feov.record.v1.SittingClose
	65,  // 34: feov.record.v1.Event.cast:type_name -> feov.record.v1.Cast
	66,  // 35: feov.record.v1.Event.dispatch:type_name -> feov.record.v1.Dispatch
	64,  // 36: feov.record.v1.Event.sitting_limit:type_name -> feov.record.v1.SittingLimit
	73,  // 37: feov.record.v1.Event.correction:type_name -> feov.record.v1.Correction
	2,   // 38: feov.record.v1.TelemetryLine.max_severity:type_name -> feov.record.v1.Grade
	28,  // 39: feov.record.v1.TelemetryLine.new_mint:type_name -> feov.record.v1.NewMint
	30,  // 40: feov.record.v1.TelemetryLine.repair_regression:type_name -> feov.record.v1.RepairRegression
	31,  // 41: feov.record.v1.TelemetryLine.edge_deltas:type_name -> feov.record.v1.EdgeDeltas
	29,  // 42: feov.record.v1.NewMint.by_severity:type_name -> feov.record.v1.SeverityTally
	74,  // 43: feov.record.v1.NewMint.by_class:type_name -> feov.record.v1.NewMint.ByClassEntry
	...
	2,   // 47: feov.record.v1.Mint.severity:type_name -> feov.record.v1.Grade
	2,   // 48: feov.record.v1.Mint.likelihood:type_name -> feov.record.v1.Grade
	2,   // 49: feov.record.v1.Mint.impact:type_name -> feov.record.v1.Grade
	2,   // 50: feov.record.v1.Mint.complexity_cost:type_name -> feov.record.v1.Grade
	6,   // 51: feov.record.v1.Mint.class_material:type_name -> feov.record.v1.ClassMaterial
	6,   // 52: feov.record.v1.ClassNew.material_default:type_name -> feov.record.v1.ClassMaterial
	7,   // 53: feov.record.v1.Close.closure_class:type_name -> feov.record.v1.Disposition
	2,   // 54: feov.record.v1.Regrade.severity:type_name -> feov.record.v1.Grade
	2,   // 55: feov.record.v1.Regrade.likelihood:type_name -> feov.record.v1.Grade
	2,   // 56: feov.record.v1.Regrade.impact:type_name -> feov.record.v1.Grade
	2,   // 57: feov.record.v1.Regrade.complexity_cost:type_name -> feov.record.v1.Grade
	2,   // 58: feov.record.v1.Finding.severity:type_name -> feov.record.v1.Grade
	2,   // 59: feov.record.v1.Finding.likelihood:type_name -> feov.record.v1.Grade
	2,   // 60: feov.record.v1.Finding.impact:type_name -> feov.record.v1.Grade
	...
	54,  // 71: feov.record.v1.Motion.grade:type_name -> feov.record.v1.GradeMotion
	55,  // 72: feov.record.v1.Motion.petition:type_name -> feov.record.v1.PetitionMotion
	58,  // 73: feov.record.v1.Motion.direction:type_name -> feov.record.v1.DirectionMotion
	56,  // 74: feov.record.v1.Motion.docket:type_name -> feov.record.v1.DocketMotion
	...
	57,  // 83: feov.record.v1.MotionRule.docket:type_name -> feov.record.v1.DocketRuling
	...
	3,   // 86: feov.record.v1.Gate.verdict:type_name -> feov.record.v1.Verdict
	4,   // 87: feov.record.v1.Outcome.verdict:type_name -> feov.record.v1.RunOutcome
	75,  // 88: feov.record.v1.sql:extendee -> google.protobuf.FieldOptions
	75,  // 89: feov.record.v1.prose:extendee -> google.protobuf.FieldOptions
	76,  // 90: feov.record.v1.means:extendee -> google.protobuf.EnumValueOptions
	76,  // 91: feov.record.v1.closes:extendee -> google.protobuf.EnumValueOptions
	76,  // 92: feov.record.v1.ruled_by:extendee -> google.protobuf.EnumValueOptions
	76,  // 93: feov.record.v1.seat_may_file:extendee -> google.protobuf.EnumValueOptions
	76,  // 94: feov.record.v1.mass:extendee -> google.protobuf.EnumValueOptions
	76,  // 95: feov.record.v1.correct:extendee -> google.protobuf.EnumValueOptions
	77,  // 96: feov.record.v1.check:extendee -> google.protobuf.MessageOptions
	25,  // 97: feov.record.v1.sql:type_name -> feov.record.v1.Sql
	0,   // 98: feov.record.v1.correct:type_name -> feov.record.v1.CorrectionTier
	24,  // 99: feov.record.v1.check:type_name -> feov.record.v1.SqlCheck
	100, // [100:100] is the sub-list for method output_type
	100, // [100:100] is the sub-list for method input_type
	97,  // [97:100] is the sub-list for extension type_name
	88,  // [88:97] is the sub-list for extension extendee
...
func file_record_proto_init() {
	...
	file_record_proto_msgTypes[2].OneofWrappers = []any{
		...
		(*Event_Cast)(nil),
		(*Event_Dispatch)(nil),
		(*Event_SittingLimit)(nil),
		(*Event_Correction)(nil),
	}
	...
	file_record_proto_msgTypes[46].OneofWrappers = []any{}
	file_record_proto_msgTypes[47].OneofWrappers = []any{}
	file_record_proto_msgTypes[48].OneofWrappers = []any{}
	file_record_proto_msgTypes[49].OneofWrappers = []any{}
	type x struct{}
