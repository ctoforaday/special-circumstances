...
func TestAnEmptyDischargeIsRefused(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(recordpb.Word(tc.typ), func(t *testing.T) {
			err := validate(mustRun(t, recordtest.TmpRun(t)), "blue-respond-r1", tc.typ, tc.body)
			if err == nil {
...
func TestARealDischargeIsAccepted(t *testing.T) {
	...
	for _, c := range []struct {
		...
		if err := validate(mustRun(t, recordtest.TmpRun(t)), "blue-respond-r1", c.typ, c.body); err != nil {
...
func TestAReceiptMustNameItsGapAndSayWhatItChecked(t *testing.T) {
	if err := validate(mustRun(t, recordtest.TmpRun(t)), "blue-respond-r1", recordpb.EventType_EVENT_TYPE_MANIFEST_ROW, &recordpb.ManifestRow{}); err == nil {
	...
	if err := validate(mustRun(t, recordtest.TmpRun(t)), "blue-respond-r1", recordpb.EventType_EVENT_TYPE_MANIFEST_ROW,
