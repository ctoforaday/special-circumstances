...
func TestAnEmptyDischargeIsRefused(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{recordpb.EventType_EVENT_TYPE_LOG, &recordpb.Log{Type: recordpb.LogType_LOG_TYPE_REQUEST.Enum()}, "--reason"},
		...
		t.Run(recordpb.Word(tc.typ), func(t *testing.T) {
			err := validate(mustRun(t, recordtest.TmpRun(t)), "blue-respond", tc.typ, tc.body)
			if err == nil {
...
func TestARealDischargeIsAccepted(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{recordpb.EventType_EVENT_TYPE_LOG, &recordpb.Log{Text: prose, Type: recordpb.LogType_LOG_TYPE_REQUEST.Enum()}},
		...
		if err := validate(mustRun(t, recordtest.TmpRun(t)), "blue-respond", c.typ, c.body); err != nil {
...
func TestAReceiptMustNameItsGapAndSayWhatItChecked(t *testing.T) {
	if err := validate(mustRun(t, recordtest.TmpRun(t)), "blue-respond", recordpb.EventType_EVENT_TYPE_MANIFEST_ROW, &recordpb.ManifestRow{}); err == nil {
	...
	if err := validate(mustRun(t, recordtest.TmpRun(t)), "blue-respond", recordpb.EventType_EVENT_TYPE_MANIFEST_ROW,
		&recordpb.ManifestRow{GapId: proto.String("G1")}); err == nil {
...
func TestSpotCheckBareStaysAccepted(t *testing.T) {
	if err := validate(mustRun(t, recordtest.TmpRun(t)), "red-chair", recordpb.EventType_EVENT_TYPE_SPOT_CHECK, &recordpb.SpotCheck{}); err != nil {
