...
func RegisterSeat(id Identity, runVia string) (dispatch int, where string, err error) {
	...
	// THE AGENT ID IS WHAT THIS CAN MEASURE. It names the seat's trajectory, and the trajectory is
	// where the serving model is stated — read from it when a reader asks, which is any moment
	// after this one. `register` is the seat's FIRST act: a fresh trajectory's opening records are
	// all `user` turns and the model appears on the first ASSISTANT turn, which has not happened.
	//
	// Engine-observed, like run_via, and for the same reason: a seat asked which model is
	// answering it can only report what it was told.
