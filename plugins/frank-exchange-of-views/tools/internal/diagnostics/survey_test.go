...
func TestTheRootListingIsNotHavingReadTheHelp(t *testing.T) {
	p := traj(t,
		use("a", `"/tmp/x/feov-record" --seat-id red-lens-evidence --help`), res("a", rootHelp, false),
...
func TestQuotedProseCannotNominateAVerb(t *testing.T) {
	for _, tc := range []struct{ cmd, want string }{
		...
		{`/tmp/x/feov-record --run . --seat-id red-lens-evidence lens finding --key F1`, "finding"},
