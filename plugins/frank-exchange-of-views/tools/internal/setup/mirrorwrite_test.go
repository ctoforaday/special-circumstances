...
//
// MEASURED 2026-08-16, by reusing MirrorGapPatterns from the seat probe: nothing created
// `inputs/`, the os.WriteFile error was discarded, and it returned {Written: true, Files: 55} with
// nothing on disk. Real runs were masked because BuildSkeleton makes the directory earlier in
// run.go — so the failure was reachable by any other caller and invisible to all of them.
//
// IT NOW GUARDS MirrorLaw, and that is not a downgrade. The gap-pattern mirror was deleted when
// the whole-corpus staging went (the by-class index replaced it, and the skill had already
// recorded that staging the corpus was measured worthless). MirrorLaw carried the IDENTICAL
// defect, unfixed: the fix landed on one mirror in 2026-08 and its sibling was never touched,
// because nothing had reused it the way the probe reused the other. A count that outruns the disk
// is the failure — a run with no law is ordinary and says so, while one reporting a corpus it does
// not have looks complete to every reader after it.
