...
func TestTheSpanIsAttributedToTheHookAndCarriesTheAgent(t *testing.T) {
	...
	for _, e := range m.Events {
		...
		// The round is the EPOCH — the count of chair registers at the write — and a hook that fires
		// before any chair has sat is in epoch 0. That is a real answer, not an unknown: no dispatch
		// cycle has opened. There is no -1 any more because there is nothing the record cannot say.
		if e.GetRound() != 0 {
			t.Errorf("round = %d, want 0 — no chair has registered, so this is the base epoch", e.GetRound())
		}
		return
	}
