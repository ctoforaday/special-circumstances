	...
	return []scenario{
		{
			...
			cmds: []cmd{
				...
				base("blue", "revision", "--run", "{RUN}", "--seat-id", "blue-respond-r2", "--text", "first pass", "--claim-count", "132"),
				base("blue", "friction", "--run", "{RUN}", "--seat-id", "blue-respond-r2", "--text", "no PDF extraction"),
				// implicit register: a seat that never registered still records
				base("lens", "finding", "--run", "{RUN}", "--seat-id", "red-lens-r1-L5", "--label", "L5-F1",
					"--severity", "medium", "--likelihood", "high", "--impact", "medium", "--text", "unfounded leap"),
			},
		},
		{
		...
		{
			name: "class_registry", // oracle: unknown class refused with hint; --class-new extends
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("merge", "close", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--id", "R9-9", "--anchor-seat", "L1",
					"--anchor-tool", "git show", "--anchor-target", "7bc501e:x"),
				base("merge", "close", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--id", "R1-1", "--as", "closed_with_regression",
					"--anchor-seat", "L1", "--anchor-tool", "git show", "--anchor-target", "7bc501e:x"),
				base("merge", "close", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--id", "R1-1", "--as", "closed",
					"--anchor-seat", "L1", "--anchor-tool", "WebFetch", "--anchor-target", "https://example.invalid/spec#s3"),
			},
		},
		{
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("lens", "finding", "--run", "{RUN}", "--seat-id", "red-lens-r1-L1", "--label", "L1-F1",
					"--severity", "low", "--likelihood", "low", "--impact", "low", "--text", "older shard"),
				base("lens", "register", "--run", "{RUN}", "--seat-id", "red-lens-r1-L1"),
				{
					role: "lens",
					args: []string{"finding", "--run", "{RUN}", "--seat-id", "red-lens-r1-L1", "--label", "L1-F2",
						"--severity", "low", "--likelihood", "low", "--impact", "low", "--text", "newer shard"},
		...
		},
		{
			...
			cmds: []cmd{
				base("lens", "finding", "--run", "{RUN}", "--seat-id", "red-lens-r1-L1", "--label", "L1-F1",
					"--severity", "medium", "--likelihood", "medium", "--impact", "medium", "--text", "round one"),
				base("lens", "finding", "--run", "{RUN}", "--seat-id", "red-lens-r2-L1", "--label", "L1-F1",
					"--severity", "medium", "--likelihood", "medium", "--impact", "medium", "--text", "round two"),
			},
		},
		{
			name: "observations_and_disposal", // oracle: undisposed observations surface; disposal clears
			cmds: []cmd{
				base("lens", "observe", "--run", "{RUN}", "--seat-id", "red-lens-r1-L5", "--kind", "note", "--label", "N1", "--text", "below bar but real"),
				base("lens", "observe", "--run", "{RUN}", "--seat-id", "red-lens-r1-L5", "--text", "unlabelled, ordinal-keyed"),
				base("lens", "observe", "--run", "{RUN}", "--seat-id", "red-lens-r1-L5", "--text", "second unlabelled"),
				base("merge", "register", "--run", "{RUN}", "--seat-id", "red-merge-r1"),
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("merge", "close", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--id", "R1-2",
					"--anchor-seat", "L4", "--anchor-tool", "Read", "--anchor-target", "report.md#S3"),
				base("merge", "register", "--run", "{RUN}", "--seat-id", "red-merge-r2"),
				...
				base("merge", "close", "--run", "{RUN}", "--seat-id", "red-merge-r2", "--id", "R1-1",
					"--anchor-seat", "L5", "--anchor-tool", "Grep", "--anchor-target", "report.md"),
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("merge", "regrade", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--id", "R1-1", "--severity", "medium",
					"--likelihood", "low", "--basis", "blue narrowed the scope; consequence shrank"),
				base("merge", "regrade", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--id", "R1-1", "--impact", "low",
					"--basis", "second movement, same id"),
			},
		},
		{
		...
		},
		{
			...
			cmds: []cmd{
				base("merge", "register", "--run", "{RUN}", "--seat-id", "red-merge-r1"),
				base("merge", "position", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--file", "{RUN}/prose.md"),
				base("merge", "mint", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--class", "scope-creep", "--check", "x",
					"--severity", "medium", "--likelihood", "medium", "--impact", "medium", "--file", "{RUN}/prose.md"),
				base("lens", "finding", "--run", "{RUN}", "--seat-id", "red-lens-r1-L5", "--label", "L5-F9",
					"--severity", "low", "--likelihood", "low", "--impact", "low", "--file", "{RUN}/prose.md"),
			},
		},
		{
			...
			cmds: []cmd{
				base("merge", "register", "--run", "{RUN}", "--seat-id", "red-merge-r1"),
				base("merge", "position", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--file", "{RUN}/red.md"),
				base("merge", "mint", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--class", "scope-creep", "--check", "x",
					"--severity", "medium", "--likelihood", "medium", "--impact", "medium", "--problem", "docketed"),
				base("merge", "closing", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--id", "R1-1", "--text", "red's closing"),
				base("blue", "position", "--run", "{RUN}", "--seat-id", "blue-respond-r1", "--file", "{RUN}/blue.md"),
				base("blue", "closing", "--run", "{RUN}", "--seat-id", "blue-respond-r1", "--id", "R1-1", "--text", "blue's closing"),
				base("blue", "revision", "--run", "{RUN}", "--seat-id", "blue-respond-r1", "--text", "repairs landed", "--claim-count", "88"),
				...
				base("blue", "dispute", "--run", "{RUN}", "--seat-id", "blue-respond-r1", "--id", "R1-1",
					"--dimension", "likelihood", "--proposed", "low", "--evidence", "the harm needs two failures"),
				...
				base("bench", "opinion", "--run", "{RUN}", "--seat-id", "judge-r1", "--gap-id", "R1-1", "--disposition", "carried",
					"--principle", "correctness over economy", "--tension", "thoroughness vs cost",
					"--review-flag", "the figure was never recomputed", "--text", "the rationale body"),
				base("bench", "certify", "--run", "{RUN}", "--seat-id", "judge-r1", "--text", "what a human should re-examine"),
			},
		},
		{
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("bench", "opinion", "--run", "{RUN}", "--seat-id", "judge-r2", "--gap-id", "R2-1", "--disposition", "closed"),
				base("bench", "opinion", "--run", "{RUN}", "--seat-id", "judge-r2", "--gap-id", "R2-1", "--disposition", "closed",
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("lens", "finding", "--run", "{RUN}", "--seat-id", "red-lens-r1-L1", "--label", "L1-F1",
					"--severity", "medium", "--likelihood", "medium", "--impact", "high", "--location", "## S2", "--text", "citation does not support"),
				base("lens", "register", "--run", "{RUN}", "--seat-id", "red-lens-r1-L5"),
				base("lens", "finding", "--run", "{RUN}", "--seat-id", "red-lens-r1-L5", "--label", "L5-F1",
					"--severity", "high", "--likelihood", "high", "--impact", "high", "--location", "## S4", "--text", "a leap of faith"),
				base("lens", "observe", "--run", "{RUN}", "--seat-id", "red-lens-r1-L5", "--kind", "checked-held", "--label", "L5-N1", "--text", "checked, held"),
				base("merge", "register", "--run", "{RUN}", "--seat-id", "red-merge-r1"),
				...
				base("merge", "position", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--text", "round one: FAIL"),
				base("merge", "verdict", "--run", "{RUN}", "--seat-id", "red-merge-r1", "--as", "FAIL"),
			},
		},
	}
