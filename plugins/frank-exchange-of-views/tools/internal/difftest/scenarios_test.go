	...
	return []scenario{
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("close", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R1-1"),
				base("close", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R9-9", "--verified-by", "L1",
					"--verified-with", "git show", "--verified-against", "7bc501e:x"),
				base("close", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R1-1", "--as", "repaired_with_regression",
					"--verified-by", "L1", "--verified-with", "git show", "--verified-against", "7bc501e:x"),
				base("close", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R1-1", "--as", "repaired",
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("carry", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R2-1", "--carried-from", "1"),
			},
		},
		{
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("regrade", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R1-1", "--severity", "medium"),
				base("regrade", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R1-1", "--severity", "medium",
					"--likelihood", "low", "--reason", "blue narrowed the scope; consequence shrank"),
				base("regrade", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R1-1", "--impact", "low",
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("closing", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R1-1", "--reason", "red's closing"),
				base("position", "--run", "{RUN}", "--seat-id", "blue-respond", "--reason-file", "{RUN}/blue.md"),
				base("closing", "--run", "{RUN}", "--seat-id", "blue-respond", "--id", "R1-1", "--reason", "blue's closing"),
				base("revision", "--run", "{RUN}", "--seat-id", "blue-respond", "--reason", "repairs landed"),
				base("manifest-row", "--run", "{RUN}", "--seat-id", "blue-respond", "--id", "R1-1", "--reason", "figures recomputed; check run: pass"),
				base("motion", "grade", "file", "--run", "{RUN}", "--seat-id", "blue-respond", "--id", "R1-1",
				...
				base("motion", "docket", "file", "--run", "{RUN}", "--seat-id", "red-chair", "--id", "R1-1",
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("close", "--run", "{RUN}", "--seat-id", "blue-respond", "--id", "R1-1"),
		...
		},
		{
			...
			cmds: []cmd{
				...
				base("spot-check", "--run", "{RUN}", "--seat-id", "red-chair", "--ids", "R1-1, R1-2", "--reason", "both re-read"),
				base("register", "--run", "{RUN}", "--seat-id", "blue-respond"),
				base("motion", "grade", "file", "--run", "{RUN}", "--seat-id", "blue-respond", "--id", "R1-1",
