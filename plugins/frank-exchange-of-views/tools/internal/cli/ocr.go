...
func newOCRPages() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Long: "pages renders each page of the cached document --sha to a GRAYSCALE PNG under <run>/cache/<sha>.pages/ and writes a render record beside them holding the resolution, one hash per page image, and the renderer's library@semver. --dpi renders EVERY page at one resolution, 300 by default. The automatic scan-reading path does not: it follows each page's own scan resolution, floored at 300 — where the detector's constants were measured — and capped at 600, so a document of mixed scans is a range rather than one number. `ocr read` accepts any page inside that 300-600 band and refuses one outside it. Rendering the same scan here at 300 and letting the automatic path render it at its own higher resolution produces two different readings of one document; --dpi is for a human inspecting the pixels, and the reading an audit re-derives is the automatic one. It prints a summary naming the directory and the page range — never an image, and never the document. " +
...
func newOCRRead() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Long: "read runs the OCR engine compiled into this binary (tesseract + leptonica, statically linked) over each page image rendered by `ocr pages`. The assembled text is written to <run>/cache/<sha>.ocr.txt and each page's own reading beside its image. " +
			"FULLY LOCAL AND DETERMINISTIC: no model, no credentials, no network. The record keys the reading to the engine identity and the exact image hashes, so an audit can re-derive it byte for byte. It reads any page rendered between 300 and 600 DPI — the band the detector's constants are derived over, one page at a time from that page's own resolution — and refuses a page outside it; re-render if the images are lower. " +
