...
func TestAFindingAboutAnAbsenceNeedsNoBorrowedQuote(t *testing.T) {
	runDir := seatRun(t)
	if _, err := run(t, "finding", "--run", runDir, "--seat-id", "red-lens-L6",
...
func TestAFindingWithNoAnchorIsRefusedAndOffersTheOtherKind(t *testing.T) {
	runDir := seatRun(t)
	_, err := run(t, "finding", "--run", runDir, "--seat-id", "red-lens-L6",
...
func TestAnAboutReferenceIsCheckedAgainstTheRecord(t *testing.T) {
	runDir := seatRun(t)
	_, err := run(t, "finding", "--run", runDir, "--seat-id", "red-lens-L5",
