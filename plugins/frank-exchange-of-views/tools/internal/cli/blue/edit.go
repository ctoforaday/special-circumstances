...
// exact current span --old with --new while PRESERVING red's invisible finding-markers. A
// span that would drop or split a "<!--fx:...-->" marker is REJECTED (edit around it). Each
// applied edit appends a `blue_edit` event — an append-only diff-stack that replays onto the
// round-0 report to equal the current head.
//
...
func newEdit() *cobra.Command {
	...
	c.Flags().String(flags.Old, "", "REQUIRED — the EXACT current span to replace (matched across the invisible marker layer; rejected if absent or if it contains a finding-marker)")
...
// with `new`, or an error if old is absent (errMisQuote) or its span contains a
// finding-marker. The crash-reconcile and the lock live in applyEdit; the validation peek
// reuses this via validateEdit. Fuzzed directly (edit_fuzz_test.go).
func planEdit(report, old, new string) (string, error) {
	...
	if has, id := spanMarker(report[start:end]); has {
		return "", fmt.Errorf("blue edit: your --old span contains finding-marker %s — markers are immortal; edit AROUND it (split your edit so the marker sits between two edits)", id)
	}
	next := report[:start] + new + report[end:]
	if dropped := droppedMarker(report, next); dropped != "" {
		return "", fmt.Errorf("blue edit: internal error — this edit would drop finding-marker %s (report unchanged)", dropped)
	}
	return next, nil
}
...
// spanMarker reports whether span contains a finding-marker, and names the first id (a
// bare "<!--fx:" prefix with no well-formed id still rejects, named generically — never
// allow a partial marker to be split).
func spanMarker(span string) (bool, string) {
	if !strings.Contains(span, "<!--fx:") {
		return false, ""
	}
	if ids := claimcount.FindingAnchorIDs(span); len(ids) > 0 {
		return true, ids[0]
	}
	return true, "a finding-marker"
}

// droppedMarker returns a marker id present in before but absent from after, or "".
func droppedMarker(before, after string) string {
	have := map[string]bool{}
	for _, id := range claimcount.FindingAnchorIDs(after) {
		have[id] = true
	}
	for _, id := range claimcount.FindingAnchorIDs(before) {
		if !have[id] {
			return id
		}
	}
