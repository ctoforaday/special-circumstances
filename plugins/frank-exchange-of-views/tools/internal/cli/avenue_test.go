...
func TestAvenueProposalIsAssignedAnIDAndStartsProposed(t *testing.T) {
	...
	out, err := run(t, "blue", "avenue", "--run", runDir, "--seat-id", seat,
...
func TestAvenueStatusMovesAndKeepsItsSubstance(t *testing.T) {
	runDir := t.TempDir()
	seat := avenueSeat(t, runDir)
	if _, err := run(t, "blue", "avenue", "--run", runDir, "--seat-id", seat,
		"--line", "survey primality libraries", "--hypothesis", "implementations disagree at small n"); err != nil {
		t.Fatal(err)
	}
	if _, err := run(t, "blue", "avenue", "--run", runDir, "--seat-id", seat,
		"--id", "A1", "--status", "abandoned", "--reason", "every implementation agrees at n=7; the hypothesis is dead"); err != nil {
		t.Fatalf("move: %v", err)
	}
	...
	for _, want := range []string{"A1", "survey primality libraries", "implementations disagree", "the hypothesis is dead", "r1 proposed -> r1 abandoned"} {
...
func TestAvenueMoveRequiresWhatChanged(t *testing.T) {
	runDir := t.TempDir()
	seat := avenueSeat(t, runDir)
	if _, err := run(t, "blue", "avenue", "--run", runDir, "--seat-id", seat, "--line", "a line"); err != nil {
		t.Fatal(err)
	}
	_, err := run(t, "blue", "avenue", "--run", runDir, "--seat-id", seat, "--id", "A1", "--status", "abandoned")
	if err == nil {
...
func TestAvenueMoveRefusesAnUnknownID(t *testing.T) {
	runDir := t.TempDir()
	seat := avenueSeat(t, runDir)
	_, err := run(t, "blue", "avenue", "--run", runDir, "--seat-id", seat,
		"--id", "A9", "--status", "pursued", "--reason", "why")
	if err == nil {
		t.Fatal("a move against an avenue nobody proposed was accepted")
	}
	if !strings.Contains(err.Error(), "A9") {
...
func TestAvenueRefusesTheAmbiguousBothForm(t *testing.T) {
	runDir := t.TempDir()
	seat := avenueSeat(t, runDir)
	if _, err := run(t, "blue", "avenue", "--run", runDir, "--seat-id", seat, "--line", "a line"); err != nil {
		t.Fatal(err)
	...
	if _, err := run(t, "blue", "avenue", "--run", runDir, "--seat-id", seat,
		"--id", "A1", "--line", "another line", "--status", "pursued", "--reason", "r"); err == nil {
...
func TestRedCannotProposeAnAvenue(t *testing.T) {
	runDir := t.TempDir()
	if _, err := run(t, "merge", "avenue", "--run", runDir, "--seat-id", "red-merge-r1",
...
func TestOpenAvenuesAreSurfacedAsOwingADecision(t *testing.T) {
	...
	if !strings.Contains(out, "Awaiting a decision") || !strings.Contains(out, "A1") {
