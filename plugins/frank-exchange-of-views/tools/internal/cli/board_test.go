...
func TestFindingsViewProjectsLensFindings(t *testing.T) {
	...
	if _, err := run(t, "register", "--run", runDir, "--seat-id", "red-lens-logic"); err != nil {
		t.Fatal(err)
	}
	if _, err := run(t, "finding", "--run", runDir, "--seat-id", "red-lens-logic",
		"--key", "F1", "--quote", "§2", "--reason", "second", "--severity", "high", "--likelihood", "high", "--impact", "high"); err != nil {
		t.Fatal(err)
	}
	out, err := run(t, "show", "--run", runDir, "--seat-id", "red-chair", "findings")
	if err != nil {
...
func TestBoardCountsCiteEvents(t *testing.T) {
	...
	if b := board(t, runDir, "red-chair"); b.Counts.Citations != 3 {
