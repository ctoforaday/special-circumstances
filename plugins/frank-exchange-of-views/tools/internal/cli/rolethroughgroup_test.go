...
//
// `blue show worklist` reported its role as "show" from the day `show` became a group, because
...
func TestTheRoleSurvivesTheShowGroup(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(out), &got); err != nil {
		t.Fatalf("worklist did not parse: %v\n%s", err, out)
	}
	...
	var sawOpenGap bool
	for _, d := range got.Sitting.Outstanding {
		if strings.Contains(d.What, "is open") {
			sawOpenGap = true
		}
	}
	if !sawOpenGap {
		...
		t.Errorf("a merge sitting on a board with open gaps listed no open-gap duty; outstanding was:\n  %s\n\n"+
			"`verdict --as PASS` refuses over those gaps, so a worklist that omits them tells a seat it is finished while the write path holds it.",
			strings.Join(what, "\n  "))
	}
}
