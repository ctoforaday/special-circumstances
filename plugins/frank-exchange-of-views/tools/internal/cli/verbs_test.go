	...
	cases := []struct {
	...
	}{
		{
			...
			args: []string{"--claim", "the claim", "--reference", "https://example.test/a",
				"--independent", "--as", "supports", "--reason", "read at the leaf",
			...
			want: map[string]string{"claim": "the claim", "reference": "https://example.test/a",
				"outcome": "supports", "text": "read at the leaf", "access_date": "2026-07-18"},
		...
		},
		{
			...
			args: []string{"--claim", "c", "--reference", "https://example.test/b", "--independent",
				"--as", "weak", "--reason", "it gestures at it"},
			typ:    "verify",
			want:   map[string]string{"reference": "https://example.test/b", "outcome": "weak"},
