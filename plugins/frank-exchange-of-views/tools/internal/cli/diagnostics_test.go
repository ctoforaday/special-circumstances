...
func TestASurfaceThatArrivesThroughARefusalStillCounts(t *testing.T) {
	...
	if len(got.Seats) != 1 || got.Seats[0].SeatID != "red-lens-evidence" {
...
func TestWithoutASittingItRefusesRatherThanCreditingSeatsThatNeverSat(t *testing.T) {
	runDir := seatRun(t)
	for _, s := range []string{"red-lens-evidence", "red-chair"} {
	...
	for _, want := range []string{"red-lens-evidence", "red-chair", "--sitting"} {
