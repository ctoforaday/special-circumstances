...
const speedClause = ` SPEED: ONE MESSAGE IS ONE ROUND TRIP, whatever it carries. Five independent tool calls in ONE message cost one round trip; the same five in five messages cost five, and a round trip is seconds. So batch every call that does not need a prior call's OUTPUT as its input: all your projections together, independent fetches together, several file reads together. Serialize only where the second call cannot be written until the first has answered. If you are about to send a message with one tool call in it, ask what else you already know you will need. Peek and search files with the native Read (offset/limit), Grep, and Glob tools — NEVER sed/awk/head/tail/cat/grep through Bash for file access (a shell spawn costs 10-100x a native read and buys nothing). KNOWN HARNESS LIMIT (W1.11, on file three times — do NOT re-log it as friction): Glob/Grep may refuse paths outside the session's registered working directories ("Path does not exist") while Read and Bash reach them; for searches under the run directory the SANCTIONED fallback is Bash grep/ls — this is the one exception to the no-shell-file-access rule.`
...
// A seat learns its situation from its work list. No prompt here sends it anywhere else, and no
// prompt names an act: the help page owns the verbs, and a prompt that spells one competes with
// it. `priors-are-poison` supplies a cross-run seed and asserts no prompt carries a scorecard at
// all, which is what keeps this from growing back. Rationale: #1126.
...
// THE BENCH'S FOUR OCCASIONS, EACH AUTHORED ONCE AND USED BY BOTH ITS READERS — the prompt, where
// the seat copies it onto the record, and the dispatch label, where a human reads it on a board.
// They were two literals per sitting and could drift; the label said `terminal` while the record
// said nothing at all.
//
// THE WORDS ARE THE SCHEMA'S, and TestTheEngineDispatchesTheSchemasOccasions binds these four
// against the Occasion enum in both directions. A guard rather than generation, and the reason is
// that there is nothing here to generate FROM: which occasion the assembly sitting has is an
// authored fact about that call site, not a copy of a list. What CAN drift is whether the four
// words are the four the write path accepts, and that is exactly what the bind checks.
//
// THE DOCKET SITTING'S LABEL DOES NOT CARRY ITS WORD, and that is not an omission. Its label is
// `labelFor('judge')` — `judge #2 · <slug>` — whose literal head is what binds the dispatch to its
// tier class (TestDebateDispatchBindsToSeatClass reads everything before the first `·`). Wrapping
// it leaves no literal head and resolves no seat. The occasion reaches the RECORD from the prompt,
// which is where every reader now takes it from; the label is a dashboard's text.
const DOCKET_OCCASION = 'docket'
const PETITION_OCCASION = 'petition'
const TERMINAL_OCCASION = 'terminal'
const ASSEMBLE_OCCASION = 'assemble'

// THE OCCASION IS THE SECOND HALF OF THE SEAT'S IDENTITY, AND ONLY THE BENCH HAS ONE.
//
// SEAT_ID says who was asked; OCCASION says what they were asked. For every seat but the bench the
// id answers both — a lens audits, a lane researches — so the field is absent there and the record
// refuses it. The bench is one seat asked four different questions, and before this the answer rode
// only in the dispatch LABEL, which a dashboard reads and the archive does not keep.
//
// Stated as a VALUE the way SEAT_ID is, never as a flag: which flag carries it is the surface's to
// teach, and the prompt gates refuse a prompt that grows a command catalogue.
const recordClause = (seatId, occasion) =>
  `${holdingsClause()} SEAT_ID: ${seatId}.${occasion ? ` OCCASION: ${occasion} — what this sitting was convened to do. Your seat id is the same for all four of your sittings, so this is the only thing that tells them apart on the record; it is required of you where it is required of nobody else.` : ''} THE RECORD TOOL IS THE CONTRACT: every act of this seat happens through "${binDir}/feov-record", and the tool's own board is the ONLY source of truth for status. Routing around it into markdown is the failure this contract exists to prevent.
...
async function ensureSittingRecord(env, who, owed, opts) {
  ...
  log(`sitting-record (W1.7): ${who} did not attest ${owed} — re-prompting once before continuing`)
...
REPORT WRITE PATH: report.md hits a filename-keyed Write-guard — draft to your scratchpad under a name that is NEUTRAL of that guard (not report.md) and carries your seat id, then bash cp it to ${runDir}/blue/report.md, the one path ingest reads (nothing waits there; a report.md anywhere else is not the report); a direct Write of the report path fails and wastes a round-trip. FREEZE THE REPORT FIRST, once report.md is written — run the act your seat's help lists as freezing the report into the record as its base. Do this ONCE; if it refuses, record friction with the exact refusal and return sitting_record_appended honestly. Citing a source, proving a computation, counting claims and every later change read the frozen report, so each follows the freeze. Then recompute claim_count with the tool and relay the integer into the envelope; never hand-count it (two honest merges differed 2x before it was pinned). RECORD THE SITTING, LAST, with WHY you organised it as you did — what you compacted, what you kept whole, what you retired and against what — and its claim_count as the reason; sitting_record_appended is TRUE only after that event exists.${inquiryClause} ${petitionClause('blue-synthesize')}${frictionClause('blue-synthesize', 'blue')}${speedClause}${recordClause('blue-synthesize')} Return the blue envelope.`,
...
// THE RE-READ IS SCOPED HERE, WITH THE KIND THAT DECIDES IT, and not stated flat in the prompt body.
// Measured 2026-09-22: the body said "RE-READ THE FULL REPORT IN CONTEXT" unconditionally, one
// sentence after this clause explained what `unchanged` means. Six of seven lenses dispatched against
// an unmoved report re-read it whole and recorded nothing, at 7-30 calls each — the constitution had
...
THEN ASSEMBLE. It writes a SET — ${runDir}/report.md (the research), docket.md, debate.md, judgments.md, lines-of-inquiry.md, evidence.md, run.md, CHANGELOG.md, a README.md index and a tabbed report.html — and a document with nothing in it is not written at all, so a missing judgments.md means no motions were filed rather than a failure. Verify ${runDir}/report.md exists and reads correctly at the top: the verdict stamp is the outcome's, the sections are blue's and the record's. A tool cannot mis-author a synthesis surface — the TL;DR and the catechism are blue's, inside the audited report. An open gap that is not material stays open on the board and in the risk matrix; the chair's PASS listed it by class, on the record. THE AUTHORITATIVE OPEN COUNT IS THE BOARD'S, after every closure and ruling: read it back and report it as open_gaps in your envelope. Infra debts the bench named: ${JSON.stringify(infraDebts)}. Collated friction so far (report any of your own as well): ${JSON.stringify(friction)}.${holdingsClause()}${lawClause}${frictionClause('judge', 'bench')}${speedClause}${recordClause('judge', ASSEMBLE_OCCASION)} Return your envelope: a 5-line synopsis, open_gaps from the board, and your own friction if any.`,
