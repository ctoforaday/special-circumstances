...
// THE FOUR DEBATING ROLES, and the one place this script names them.
//
// It is a check, not a source: record.SeatRoles() is the source, and this list exists only so a
// call site that forgets its role argument fails at prompt-build time instead of composing a path
// to inputs/help-undefined.md. A missing staged file is invisible from inside a seat — it reads
// nothing, walks the tree, and the run looks like every run before this change.
const SEAT_ROLES = ['blue', 'lens', 'merge', 'bench']
...
const recordClause = (seatId, role) => {
  // THE ROLE IS PASSED, NOT DERIVED. Recovering it from the seat id here would be a second copy
  // of the tool's roleSeats prefix table living in JavaScript, and a wrong recovery would name a
  // staged file that does not exist — a seat then reads nothing and walks the tree anyway, which
  // is the tax this clause exists to remove, arriving silently. Every call site already knows its
  // role (frictionClause takes it), so it is cheaper to hand it over than to infer it.
  if (!SEAT_ROLES.includes(role)) {
    throw new Error(`recordClause: seat ${seatId} was given role ${JSON.stringify(role)}, which is not one of ${SEAT_ROLES.join(', ')} — the staged help path would name a file setup never wrote`)
  }
...
THE HELP IS THE ONLY PAGE THAT INSTRUCTS, AND READING IT IS REQUIRED — not a suggestion, and not for when you are stuck. READ YOUR WHOLE SURFACE BEFORE YOU CHOOSE, NOT THE ONE PAGE FOR THE VERB YOU ALREADY PICKED: deciding first is what makes the reading pointless. Steps 1 and 2 happen ONCE, together, at the start. Step 3 happens per act.
  ...
  3. "${binDir}/feov-record" --seat-id ${seatId} <group> <command> --help — BEFORE you run it. Step 2 is the MAP; this is the DETAIL, where a flag's exact wording lives at the moment you pass it.

MEASURED, WHICH IS WHY STEP 2 IS A FILE AND NOT A WALK. Across nine sittings seats opened 6 of 51 group pages, and 18 of the 23 they did open were for verbs already chosen — confirming a decision, not surveying a surface. And the walk ate the run: about a THIRD of all record-tool traffic in both 2026-08-23 runs was help. The duty has not been relaxed; the walk has been done for you, once, and written down.

ONE VERB PER BASH CALL — no && or ; chaining record calls, no wrapper script. MEASURED: three refusals were swallowed inside exit-0 batched shell, so the tool refused, the shell reported success, and the seat carried on believing the act had landed. A refusal you cannot see is worse than a failure, because you will not retry it. This is the one exception to batching independent calls: these are not independent of their own error reporting.
...
OWNERSHIP BINDS, AS IT DID IN ROUND 0: you write ONLY your own surfaces — TL;DR, Catechism, technical foundations, analysis, open questions, your citations. You MUST NOT introduce a \`**Verdict:**\` line or any tool-owned section (\`## Risk matrix\`, \`## Red team findings\`, \`## The debate\`, a \`## Blue team report\` wrapper) while repairing: assembly composes those from the record, you cannot know the outcome or red's findings, and anything you add there only lands stale and gets stripped.
