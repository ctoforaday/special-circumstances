...
await ensureRoundRecord(blueEnv, 'blue-synthesize', `your round-record event (blue revision) stating the synthesis and its claim_count`,
...
while (!halted && round < maxRounds) {
  ...
  const consolidatedClause = round === 1 ? '' : ` CONSOLIDATED CITATION SEAT (W2i): from round 2 the citation seats are deliberately fewer, because the ledger means a claim verified HIGH does not un-verify — your round's work is the NEW and the STALE surface, not the whole corpus again. Your duty is three things, in order: (1) verify every claim in your slice that is NEW or whose section CHANGED this round (\`show changes\` names them — the recorded edits, not blue's account of them); (2) re-fetch everything in your slice the ledger's staleness triggers fire on (>2 rounds since verification, volatile source, access-date drift); (3) SPOT-CHECK a sample of your slice's already-verified pairs — your discretion which, and reopen any that has drifted. COVERAGE IS AN OBSERVABLE, NOT AN ASSUMPTION: end your pass with a COVERAGE line stating what you verified, what you sampled, and what you left unexamined this round — an unstated gap in coverage is indistinguishable from a clean sweep, and the consolidation is only sound while the gap is visible.`
...
A PROOF IS AUDITED BY RE-RUNNING IT, NOT BY READING IT — "${binDir}/feov-record" lens reproduce --id <the proof's sha256>. Where blue backed a sentence with a computation, you re-execute the cached script and compare: this is the ONE audit that does not end in you believing bytes someone else chose. A proof recorded observed rather than reproducible did not reproduce for blue either — treat it as a measurement of a moving system, and say so in your finding if the report reads it as a proof. RE-RUNNING IS NOT ENOUGH, AND THIS IS THE POINT OF THE VERB: it measures whether the script is DETERMINISTIC, not whether it PROVES anything. A script that prints "7 is prime" reproduces perfectly forever. So READ THE SCRIPT and judge it — --as sound|unsound, with --reason saying what it ACTUALLY COMPUTES. It does not have to be pretty or idiomatic; it has to compute the thing it claims, readably enough that you can see that it does. A proof that RE-RUNS CLEAN AND ESTABLISHES NOTHING is the dangerous case, because it looks maximally credible; the report names that cell explicitly. The tool records whether it reproduced (computed by comparing bytes, never claimed by you); you record whether it proves the claim. A disagreement is still YOUR finding, in your words, through the finding verb. RULE ON BLUE'S PROPOSED DIRECTIONS — "${binDir}/feov-record" motion inquiry rule --id <Q1> --as endorsed|out-of-scope|too-thin --reason "<why>" (the id is the LINE's own — a direction has no separate filing, because blue's proposal IS the filing). Blue proposes lines of inquiry with a hypothesis; you say which are worth this run's time — endorsed, out-of-scope (a real question, but not THIS question), or too-thin (in scope, but the hypothesis does not carry its budget). Measured: blue rejected 18 of its own 86 lines of inquiry across six runs and red rejected NONE, because red had no verb to. A ruling is an ARGUMENT, not a command: it needs a reason, and blue may press it through "motion inquiry appeal --id <Q1> --reason ..." — which records the disagreement whether or not blue also pursues the line, so arguing and then yielding is on the record instead of vanishing. YOU DO NOT PROPOSE DIRECTIONS — when you want research done, that is a gap whose required_fix says so, and it is graded, tracked and closed. PROPOSE EXACT TEXT WHEN THE DEFECT IS TEXTUAL. --fix is prose (what must become true) and stays the channel for substantive work — research it, enumerate it, qualify it. For a TEXTUAL defect (an overclaim, a wrong figure, a contradiction) you MAY additionally pass --quote "<the exact current span>" --new "<what it should become>". The tool validates the pair against the LIVE report and refuses it unless the span is present and unique, so you cannot state one without reading the text you are prescribing against — which is the check whose absence caused all three of the 2026-08-04 smoke's round-2 gaps. It also refuses a replacement more than 120 characters longer than the span it replaces: that size is a substantive ADDITION and it is BLUE'S to author, not yours. You never type a basis — the tool derives fix_basis: verified from the pair validating, proposed from prose alone. DID BLUE ACTUALLY DO WHAT YOU ASKED? Before closing a gap you prescribed a fix for, put the two side by side — "${binDir}/feov-record" merge show changes --run ${runDir} --id <gap>: it prints your required_fix and acceptance_check above the exact old->new spans blue recorded against them. This does NOT replace the lens re-read of the report — a span out of context misleads on research prose — it replaces INFERRING whether your prescription was answered. Two shapes worth naming: NONE recorded against an open gap (blue answered nothing, or answered without --answers), and a change that does not meet your own acceptance_check. ESTOPPEL IS ENFORCED NOW, NOT ADVISED: if the defect you are about to raise lives in text blue applied VERBATIM from your own --new, the mint is REFUSED and the refusal is logged as friction. You are not silenced — argue it on the ORIGINAL gap, where your prescription sits beside your complaint, or mint with --supersedes <that gap> so the lineage is explicit. What you may not do is open a clean slate against your own words. And when blue applied your exact text, your prescription HAS been satisfied at that site by construction: close the gap unless you can state what your own acceptance_check still fails. The guard only catches text blue took verbatim — text blue COUNTER-EDITED is blue's authorship and you audit it normally. This is an amendment to the gap that prescribed it, not a fresh gap — 3 of 3 round-2 gaps in the 2026-08-04 smoke were text blue added in round 1 because red asked for it. Decide the binary verdict — PASS only when every remaining unadjudicated gap is closed, rebuttal_sustained, or risk_accepted. CITATIONS_CHECKED IS THE RECORD'S, NOT YOURS: set the envelope's citations_checked to counts.citations from the board (\`"${binDir}/feov-record" merge show board --run ${runDir}\`) — the tool counts the cite events the lenses recorded; never hand-count or estimate it. Record your round-${round} narrative on the RECORD as a position event — "${binDir}/feov-record" merge position --reason "<your RED narrative>" (--reason-file <path> for long prose); it renders as the round's "### RED" section from the record. CLOSING ARGUMENTS: any gap you RE-RAISE from a prior round, any successor you mint via supersedes, and any grade dispute you REJECT is docket-bound — for EACH such item, record a closing event — "${binDir}/feov-record" merge closing --id <gap> --reason "<your case, ~120 words>"; it renders as "### RED CLOSING" from the record and the judge reads it there: your strongest evidence the gap is real and graded correctly, and your answer to blue's best rebuttal so far. The judge rules AFTER blue responds this round, on the closings, the transcript, and the final artifact state only — your closing is your case; overstatement the record does not support counts against you.${petitionClause(`red-merge-r${round}`)}${reliefFor('red')}${frictionClause(`red-merge-r${round}`, 'merge')}${speedClause}${recordClause(`red-merge-r${round}`, 'red-merge')} Return the red envelope.`,
...
if (terminalDisputes.length > 0) {
  const terminalJudge = await agent(
    `Terminal dispute disposition for topic "${topic}" (debate ended ${verdict} after round ${round}; this docket fires at the exit boundary). Undisposed grade disputes: ${JSON.stringify(terminalDisputes)}. Read the transcript in full from the RECORD (feov-record bench show debate --run ${runDir}), and pull the board through the tool: feov-record bench show board --run ${runDir} --format markdown. ${lawClause}${declareClause}${inspectionClause} The resolution set at a terminal exit EXCLUDES carried — rule each dispute grade_adjusted (state the corrected grade in the rationale; assembly records the delta) or unresolved (the contested grade ships, recorded as contested in the report). deadlock: false. Record your terminal ruling as an opinion event (feov-record bench opinion --id <gap> ...) — it renders as "### LEAD (terminal disputes)" from the record.${frictionClause('judge-terminal', 'bench')}${speedClause}${recordClause('judge-terminal', 'bench')} Return the judge envelope.`,
...
const assembleEnv = await agent(
  `Final assembly for topic "${topic}", run directory ${runDir}. Debate outcome: ${verdict} after ${round} round(s)${deadlocked ? ' by judged deadlock' : ''}${exhausted ? ' by safety ceiling' : ''}. THE REPORT IS ASSEMBLED FROM THE RECORD — you author NOTHING, you copy NOTHING, you fill in NO inputs. After registering, exactly two tool calls in order: (1) record the terminal verdict as a fact — "${binDir}/feov-record" bench outcome --run ${runDir} --seat-id <your SEAT_ID> --as ${verdict}${deadlocked ? ' --ended deadlock' : exhausted ? ' --ended ceiling' : ''} --reason "<HOW THIS RUN ENDED, IN YOUR WORDS — required, as it is on every claim and judgment act here. The VERDICT is derived from the record and needs no defence; this is your account of the sitting, and where the run ended by judged DEADLOCK it is the only evidence that determination will ever have, because a deadlock leaves nothing on the record a later reader can consult>"; (1a) IF YOU HOLD SOMETHING THAT BINDS HOW THE RECORD IS READ BUT MOVES NO GAP — a construction of a term, a correction of what the record MEANS without touching what it says, or a holding you want promoted to precedent — DECLARE IT: "${binDir}/feov-record" bench declare --run ${runDir} --seat-id <your SEAT_ID> --reason "<the holding>". It renders under ### LEAD in \`show debate\`, which is where both seats read. This verb exists because a bench once had exactly such a finding, found that \`opinion\` demands a gap id and a fate it did not want to move, and put the holding in a petition ruling instead — the channel least likely to be read; (1b) IF ANYTHING IN THIS RUN NEEDS A HUMAN TO RE-EXAMINE IT — an unresolved tension, a claim that held only because nobody could reach the source, a boundary you ruled close to — SAY SO: "${binDir}/feov-record" bench certify --run ${runDir} --seat-id <your SEAT_ID> --reason "<what you would want a human to look at, and why>". A certification is NOT a verdict and does not change one: a VERIFIED run can carry it. You keep no memory between runs, so this statement is the only continuity you have, and it was named in no prompt until 2026-08-13 — the capability existed and nobody was told; (2) "${binDir}/feov-record" bench assemble --run ${runDir}. The assembler composes ${runDir}/report.md FROM THE EVENT LOG (the verdict stamp from the outcome event you just wrote, the graded risk matrix, the expansions and alternatives-considered from the line of inquiry record, red's findings in full, and the entire debate transcript — positions, closings, grade disputes, the bench's opinions and petition rulings, the terminal halt/certification) and LIFTS blue's audited sections VERBATIM from blue/report.md (the title, the TL;DR, the Catechism, the technical foundations, the analysis, the open questions). A tool cannot mis-author a synthesis surface — the run-5 catechism defect (6/7 answers regressed when assembly authored the catechism) is structurally impossible, and the TL;DR is blue's, inside the audited report, rather than a synthesis surface written after red's final audit where nothing checks it. There are NO <FILL> fields and NO sections for you to write; do not hand-write report.md and do not copy anything yourself. Verify ${runDir}/report.md exists and read its top — the verdict stamp is the outcome event's, the sections are blue's and the record's. AUTHORITATIVE OPEN COUNT: after assembling, run "${binDir}/feov-record" bench show board --run ${runDir} and report its counts.open as open_gaps in your envelope — this is the run's true outstanding-gap number (the board after every closure and ruling), and it is the ONLY correct source for it. Collated friction so far (report any of your own as well): ${JSON.stringify(friction)}.${frictionClause('assemble', 'bench')}${speedClause}${recordClause('assemble', 'bench')} Return your envelope: a 5-line synopsis, open_gaps from the board, and your own friction if any.`,
