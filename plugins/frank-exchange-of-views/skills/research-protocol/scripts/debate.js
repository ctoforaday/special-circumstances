...
const RED_ENVELOPE = {
  ...
  properties: {
    ...
    // a `lens cite` event already carries — so two channels reached one act and could disagree
    // with nothing to reconcile them. The trigger map resolved this collapse (cite events are
    // canonical; blue reads `--view citation-ledger`) and it was never executed: the array stayed
...
// `feov-record <role> show --run <dir> --view ledger|archive|board`, which computes-and-returns
