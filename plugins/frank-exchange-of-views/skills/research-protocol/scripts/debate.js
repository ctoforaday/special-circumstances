...
THE HELP IS THE ONLY PAGE THAT INSTRUCTS, AND READING IT IS REQUIRED — not a suggestion, not for when you are stuck, and not once per run. YOUR SEAT ID SELECTS YOUR SURFACE, AND YOU STATE IT ONCE: \`register\` is your first act, it binds ${seatId} to you on the record, and every call after it resolves your seat for you — so you type --seat-id at register and never again, and a value disagreeing with what you registered as is refused rather than obeyed. Until you have registered nothing says who is asking, which is why the help reads below carry it. What comes back IS what you may do: there is no wider tree behind it and nothing of yours is filed under another seat. You do NOT pass the run directory at all — it is injected on every call the tool receives, and typing an absolute path you can mistype is the thing that injection replaced. READ THE WHOLE TREE BEFORE YOU CHOOSE, NOT THE ONE PAGE FOR THE VERB YOU ALREADY PICKED. Steps 1 and 2 happen ONCE, TOGETHER, IMMEDIATELY AFTER \`register\` — before you have decided what to do, because deciding first is what makes the reading pointless. Step 3 happens per act.

  1. "${binDir}/feov-record" --seat-id ${seatId} --help — every verb your seat can run, each with what it is for. Some of those entries are GROUPS, holding commands this page does not show you.
...
MEASURED, WHICH IS WHY STEP 2 CHANGED SHAPE. Across nine sittings seats opened 6 of 51 group pages — twelve per cent — and 90% of the commands they ran were run without ever reading that command's own page. The rule they were following said to open a group "before using any command in it", so a seat that obeyed it perfectly still only ever opened the page for a verb it had already chosen: eighteen of the twenty-three pages opened were for verbs the seat went on to run. That is not surveying a surface, it is confirming a decision, and a decision made before reading is made from memory and from this prompt's vocabulary — which the paragraph below tells you is not authoritative.
...
while (!halted && round < maxRounds) {
  ...
  redEnv = await agent(
    `Red merge, round ${round}. YOUR FIRST READ — WHICH COMES AFTER THE TREE WALK BELOW, NOT BEFORE IT: read this round's lens findings from the RECORD — the \`findings\` projection — structured JSON carrying each finding's label, seat, round, role, grades, location and text. That view IS the channel: the lenses record findings as events, and there are no candidate files to read.
