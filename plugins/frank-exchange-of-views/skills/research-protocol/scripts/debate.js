...
const scorecardClause = () => (
  ...
  // and salience-priming.
  //
  // THERE IS NO `scorecards` ARG. It was destructured and never read again, while the skill told
  // the lead to paste inputs/scorecards.json into the Workflow args on every run. The simulator
  // still SUPPLIES it in `priors-are-poison` and asserts no number reaches a prompt, so
  // re-introducing it cannot leak.
  //
