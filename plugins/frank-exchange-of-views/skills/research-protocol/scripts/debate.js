...
ANSWER EVERY OPEN GAP ADDITIVELY, THROUGH THE TOOL — expand and repair where red is right, REBUT IN WRITING WITH EVIDENCE where red is wrong, and argue risk-acceptance where the fix's complexity exceeds its likelihood x impact. THE REPORT IS THE RECORD NOW (#709): there is no blue/report.md file to open. READ the living report through the tool, and CHANGE it ONLY through the tool's edit and evidence acts — replacing a quoted span, anchoring evidence, retiring a claim on the record — each an appended event the report is replayed from; a raw Write of the report is both impossible and unnecessary (your seat's help names every act). Compact and reorganize prose as clarity demands through a sequence of span replacements — but a CLAIM leaves only by being retired on the record, which names it, why it goes, and what replaces it. Losing one in a rewrite is the failure the additive rule exists to prevent. PROPAGATE EVERY CORRECTION TO ALL SITES that state the corrected claim, not only the flagged sentence — incomplete propagation was run 3's dominant blue failure class, 5 regressions in 5 rounds — and remember that an index of footnoted claims cannot see an unfootnoted restatement of one, so a bare corrected FIGURE needs a report-wide sweep of its own. Log the sites you checked in your round record.${contested.length ? ` CLOSING ARGUMENTS: the following are DOCKETED for adjudication AFTER your response this round: ${JSON.stringify(contested)}. For EACH, after your repairs, argue in ~120 words why your response resolves it, or why red's grade or claim is wrong, citing the exact section and evidence. This is your case; material not in the record cannot help you.` : ''}
...
YOUR RULING BASIS IS CONFINED TO THREE THINGS: the two sides' closings, the full transcript, and the final state of the artifacts — the board and the living report as they now stand (the report is the record now (#709) — read it THROUGH THE TOOL, not by opening blue/report.md, which no longer exists). Weigh each closing as that side's best case, and a claim in a closing that the record does not support counts AGAINST the side that made it. For every ruling on a gap with a lineage chain, READ THE NAMED ANCESTORS' RECORDS first and NAME what you read in your rationale.${lawClause}${declareClause}
