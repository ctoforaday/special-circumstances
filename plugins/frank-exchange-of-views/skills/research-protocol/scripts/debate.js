...
const recordClause = (seatId, tool) => binDir
  ? `${scorecardClause(tool)} SEAT_ID: ${seatId}. THE RECORD TOOL IS THE CONTRACT (migrated 2026-07-19 — the tool is authoritative, not "under test"): every board act happens through "${binDir}/feov-record" ${RECORD_ROLE[tool]}, and the tool's board — read it back with \`${RECORD_ROLE[tool]} show --view board\` (structured JSON) — is the ONLY source of truth for status. FIRST ACTION: "${binDir}/feov-record" ${RECORD_ROLE[tool]} register --run ${runDir} --seat-id ${seatId} — then the matching verb for each act. YOUR CONTRACT IS \`feov-record ${RECORD_ROLE[tool]} --help\` and each verb's own --help: what is listed you may do, what is NOT listed does not exist for you. Do NOT improvise a flag, invent a verb, or hand-write a board artifact the tool renders (ledger, archive, findings) — routing around the tool into markdown is the exact failure this migration removes. If you need something the tool does not offer, record it with the friction verb: a missing capability is a finding about the tooling, never a reason to hand-write. Prose payloads over 2KB go via --file, NEVER inline.`
...
phase('Assemble')
// assemblyExtra: the optional inputs the seat cannot know but the orchestrator holds, folded
// into one JSON fragment so the assembly-inputs object the seat writes stays flat.
const assemblyExtra = [
  verdict === 'HALTED' ? `,"halt_opinion":${JSON.stringify(typeof haltOpinion === 'string' ? haltOpinion : JSON.stringify(haltOpinion))}` : '',
  petitionLog.length ? `,"petition_log":${JSON.stringify(petitionLog.map((x) => (typeof x === 'string' ? x : JSON.stringify(x))))}` : '',
  gradeAdjustments.length ? `,"grade_adjustments":${JSON.stringify(gradeAdjustments.map((x) => (typeof x === 'string' ? x : JSON.stringify(x))))}` : '',
  infraDebts.length ? `,"infra_debts":${JSON.stringify(infraDebts.map((x) => (typeof x === 'string' ? x : JSON.stringify(x))))}` : '',
].join('')
await agent(
  `Final assembly for topic "${topic}", run directory ${runDir}. Debate outcome: ${verdict} after ${round} round(s)${deadlocked ? ' by judged deadlock' : ''}${exhausted ? ' by safety ceiling' : ''}. THE TOOL ASSEMBLES THE REPORT NOW: "${binDir}/feov-record" bench assemble copies the verbatim sections (Catechism, technical foundations, analysis) from blue/report.md, embeds blue's report and red's board IN FULL, generates the graded risk matrix from the board's open gaps, stamps the verdict (including the CEILING/HALTED terminal states), and carries the lines-of-inquiry and open questions. A tool CANNOT mis-author the catechism — the run-5 defect (an assembly-authored catechism regressed 6/7 answers) is made structurally impossible. You do NOT hand-write report.md and you do NOT copy sections yourself. YOUR JUDGMENT is TWO things only: (1) the TL;DR — read ${runDir}/blue/report.md and write 3-6 sentences (the answer, the confidence, the sharpest caveat); (2) the per-round synopsis — read the "### LEAD"/"### RED"/"### BLUE" sections of ${runDir}/debate.md and write ONE line per round (the literal transcript stays in debate.md). Write your assembly inputs as JSON to <your session scratchpad>/assembly-inputs.json (ABSOLUTE scratchpad path, never under ${runDir}), filling ONLY the two <FILL> string fields and changing nothing else: {"verdict":"${verdict}","topic":${JSON.stringify(topic)},"rounds":${round},"deadlocked":${!!deadlocked},"exhausted":${!!exhausted},"open_questions":${JSON.stringify((blueEnv && blueEnv.open_questions) || [])}${assemblyExtra},"tldr":"<FILL: your 3-6 sentence TL;DR>","synopsis":"<FILL: one line per round>"}. Then run "${binDir}/feov-record" bench assemble --run ${runDir} --inputs <your scratchpad>/assembly-inputs.json — it writes ${runDir}/report.md. Verify report.md exists and read its top; the stamp and the risk matrix are the tool's, not yours. Collated friction so far (report any of your own as well): ${JSON.stringify(friction)}.${frictionClause('assemble')}${speedClause}${recordClause('assemble', 'bench')} Return a 5-line synopsis of the final report, plus your own friction if any.`,
