...
// MIGRATED 2026-07-19: the ledger and archive are now the TOOL's renders, not hand-written
// files. red-merge mints/closes through feov-record and runs `merge render`; every seat that
// reads the board (blue for open gaps, the judge to rule, assembly to copy) reads the tool's
// render here. One redirect carries every downstream reader from hand-written markdown to the
// authoritative record — nobody's read path changes, only where the bytes come from.
const LEDGER = `${runDir}/records/render-shadow/ledger.md`
const ARCHIVE = `${runDir}/records/render-shadow/archive.md`
...
Decide the binary verdict — PASS only when every remaining unadjudicated gap is closed, evidence-rebutted, or risk-accepted. Append the round-${round} "### RED" section to ${runDir}/debate.md per the debate template. CLOSING ARGUMENTS: any gap you RE-RAISE from a prior round, any successor you mint via supersedes, and any grade dispute you REJECT is docket-bound — for EACH such item, also append a "### RED CLOSING (round ${round})" entry to ${runDir}/debate.md — max 120 words per item: your strongest evidence the gap is real and graded correctly, and your answer to blue's best rebuttal so far. The judge rules AFTER blue responds this round, on the closings, the transcript, and the final artifact state only — your closing is your case; overstatement the record does not support counts against you.${petitionClause(`red-merge-r${round}`)}${frictionClause(`red-merge-r${round}`)}${speedClause}${recordClause(`red-merge-r${round}`, 'red-merge')} Return the red envelope.`,
  ...
  // DISABLED 2026-07-19 (smoke wf_f5b6b776). These two gates trust red-merge's
  // SELF-REPORTED counts (archive_blocks, ledger_closure_lines) — and the 2026-07-19
  // haiku smoke proved them untrustworthy: red-merge wrote 38KB of markdown, drove the
  // tool for register+position ONLY (0 gaps), and self-reported archive_blocks:22 in a
  // round-1 run where the true archived count is 0. The gate killed the run comparing two
  // numbers the seat made up.
  //
  // The orchestrator is sandboxed and cannot read the record, so it cannot derive the true
  // counts here — only a SEAT (reading `feov-record merge show --view board` and returning
  // the count) or CAPTURE (post-run, reads the git-tracked shards) can. These gates are
  // retired pending that migration: the merge seat drives the tool, reads its own board
  // back for counts/verdict, and capture audits the truth from disk. See
  // plans/tool-is-the-contract.md.
  //
  // if (round >= 2 && prevArchiveBlocks > 0 && (!redEnv.archive_spot_checks || redEnv.archive_spot_checks.length === 0)) throw ...
  // if (redEnv.ledger_closure_lines !== redEnv.archive_blocks) throw ... [self-inconsistent self-report]
...
phase('Assemble')
await agent(
  `Final assembly for topic "${topic}", run directory ${runDir}. Debate outcome: ${verdict} after ${round} round(s)${deadlocked ? ' by judged deadlock' : ''}${exhausted ? ' by safety ceiling' : ''}. Assemble ${runDir}/report.md by UNION per the report template (references/report_template.md): verdict stamp, TL;DR, the Catechism COPIED VERBATIM from blue/report.md's audited "## The Catechism" section (UNION-COPY, NEVER AUTHORSHIP — the run-5 post-capture audit found an assembly-authored catechism DEFECTIVE with 6/7 answers regressing to pre-repair phrasings; if blue's section is missing, say so in one line and DO NOT write one yourself), technical foundations, analysis, graded risk matrix (including risk_accepted items with rationale), then blue/report.md IN FULL, red's board IN FULL (${LEDGER} then ${ARCHIVE}), per-round debate synopsis pointing at debate.md, the "## Lines of Inquiry" section copied from ${runDir}/records/render-shadow/lines-of-inquiry.md if it exists (the exploration space — what was pursued, abandoned and declined; a report that shows only its conclusions hides the roads not taken, and the abandoned ones are what a future run needs most), an "Open questions carried past this run" section from blue's final envelope: ${JSON.stringify((blueEnv && blueEnv.open_questions) || [])}, and the consolidated footnotes. Never compress the research into a digest. ${verdict === 'UNVERIFIED' ? 'Stamp UNVERIFIED and list every outstanding gap with its disposition and the compromise rationale.' : ''}${verdict === 'CEILING' ? ' Stamp CEILING-TERMINATED at the top: the run hit its round ceiling while still converging, so this is NOT a judged failure to verify and must not be read as one. State in its own section: which gaps remain open, that the final blue revision was never audited by a red pass, and the re-audit obligation this carries OUT of the run — the debt is the finding.' : ''}${verdict === 'HALTED' ? ` Stamp HALTED at the top: the bench ended this run. Quote the halt opinion IN FULL and VERBATIM — it is relayed to the human, never smoothed: ${JSON.stringify(haltOpinion)}.` : ''}${petitionLog.length ? ` JUDICIAL RECORD — petitions (list each with its ruling and opinion, never dropped): ${JSON.stringify(petitionLog)}.` : ''}${gradeAdjustments.length ? ` Terminal grade adjustments ruled by the judge (record each delta in the risk matrix): ${JSON.stringify(gradeAdjustments)}.` : ''}${infraDebts.length ? ` INFRASTRUCTURE DEBTS ruled routed_to_infrastructure (W1.9 — the lead's owed fixes; list them in their own report section, never dropped): ${JSON.stringify(infraDebts)}.` : ''} Collated friction so far (report any of your own as well): ${JSON.stringify(friction)}.${frictionClause('assemble')}${speedClause}${recordClause('assemble', 'bench')} Return a 5-line synopsis of the final report, plus your own friction if any.`,
