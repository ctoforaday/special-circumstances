...
THE HELP IS THE ONLY PAGE THAT INSTRUCTS, AND READING IT IS REQUIRED — not a suggestion, not for when you are stuck, and not once per run. READ YOUR WHOLE SURFACE BEFORE YOU CHOOSE, NOT THE ONE PAGE FOR THE VERB YOU ALREADY PICKED. Do it ONCE, IMMEDIATELY AFTER \`register\` — before you have decided what to do, because deciding first is what makes the reading pointless:
  ...
  "${binDir}/feov-record" --seat-id ${seatId} manual — every command on your surface, each under a header naming it and followed by that command's own help, run live. It is long: send it to a file under your session scratchpad (an ABSOLUTE path, never under the run directory) and read that file whole.
...
let blueEnv = await agent(
  `Blue synthesis for topic: "${topic}". PRE-FLIGHT: if ${runDir}/inputs/red-gap-patterns.md exists, read it before merging — check your synthesis against red's known gap patterns. Read every draft in ${runDir}/blue/candidates/ and synthesize ${runDir}/blue/report.md by UNION: deduplicate overlapping claims, reorganize freely, and let no CLAIM leave without a record. Compacting and reorganizing prose is not subtraction — rewriting for density is encouraged; losing a claim in the rewrite is the failure, and substance leaves only by being retired on the record, which names the claim, why it goes, and what replaces it. THE REPORT CARRIES RESEARCH PROSE AND THE TOOL'S OWN MARKERS, AND NOTHING ELSE: no provenance or attribution tags, no notes addressed to another seat, no instructions, no argument about the run, no narration of how the report was made. It has exactly one reader and they came for the subject. WHERE A CLAIM CAME FROM IS ALREADY ON THE RECORD — the lane drafts, the frozen base and every recorded edit are all there — so provenance is EXTRACTED from the record by anyone who wants it, never read back out of prose somebody remembered to write. Operational facts belong on the log, arguments belong in your position, and what LIMITS THE CONCLUSION stays in the report, re-voiced as a limit on the answer rather than a story about the attempt.
