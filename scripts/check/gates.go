...
func gateSet() []gate {
	...
	gs = append(gs,
		// The corpus reads real scans through the real engine: minutes of work, in its own
		...
		gate{id: "fetch-bin-platforms", kind: kindExtern, dir: ".", ciJob: "fetch-bin-platforms",
			skip: "needs macOS and Windows runners; on this host the doctor package's fetch-bin contract tests drive the same script",
			why:  "the hooks' fetcher must install on every OS a consumer runs, and its fetch must outlive the guard"},
