...
// Package buildid is the ONE authored copy of the build-provenance helpers.
//
// Every plugin's tools/ must build standalone — `sc-doctor --fix` runs
// `go build -C <installedPluginRoot>/tools ./cmd/<name>`, and a consumer with one plugin
// installed has only that plugin's tree — so the modules cannot import a shared package.
// "Universal" therefore means one authored source mechanically propagated, not one import:
// scripts/buildidgen writes a copy into each module and `-check` fails when a copy drifts.
//
// Hand-written copies of this file had already begun to diverge — one module carried ExeName
// and the other hand-rolled the same conditional in a test, because it had no ExeName to call.
package buildid
