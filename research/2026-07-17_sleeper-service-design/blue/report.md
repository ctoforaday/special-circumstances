...
**Round 2 revision (2026-07-17):** all 22 round-2 gaps addressed — per-gap log in
`blue/CHANGELOG.md` Round 2. The board's center was the round-1 repair machinery itself;
the design answer is invariant 7 (§0), stated once per the lead's direction: every
wrapper gate emits a checked, human-surfaced liveness/outcome record. Design deltas of
record: the wrapper becomes a PHASE-DRIVEN stream-json session driver (flag verified on
the pinned CLI — R2-1); fence liveness is proven per run by the guard's fired-record,
closing the layer-1-masks-layer-2 hole (R2-2); the FEOV execution locus is
probe-determined MIXED (session-Bash setup/capture re-hosted wrapper-side; the debate
engine rides the Workflow tool, argued and hook-scoped — R2-3); origin-tagging moves
from name-glob to wrapper-stamped causal-provenance markers (R2-5) with a DECIDED
severity-gated corroboration bar (R2-6); the tamper-evidence snapshot gains its watchman
(wrapper compare + TAMPER flag + fail-closed next preflight — R2-7); the executing
plugin copy gains a hash-verified lifecycle (R2-8); the dead-man surface gains a
SessionStart push line and an honest latency statement (R2-9); per-cause HALT bounds
deterministic death spend (R2-10); graduation-queued stubs no longer auto-stale (R2-11);
the nightly PDF-tooling trade is chosen and stated (R2-17); the cap arithmetic is
reconciled with expected spend (R2-18).
...
status:            <open | graduation-queued | stale | graduated | rejected — "open" at
                    mint; harvest auto-stales UNTRIAGED (open) stubs at 30 days computed
                    from the dated filename (R1-22); human-set `graduation-queued` is
                    exempt from auto-stale while still deduping its class (R2-11);
                    humans set graduation-queued and the terminal states>
...
(`research/<date>_self-improve/`); on start, if today's run is recorded COMPLETE in the
wrapper's own step-7 ledger record (operator-owned dir — the completeness signal is never
read from loop-writable run-dir contents: R2-12), exit
0; if started-but-unrecorded, resume it (`--resume` from the recorded session id) instead
of starting fresh. A dropped day is a no-op; a double fire is a resume. **Resume cap (round 1,
...
| 3 | Proposal fatigue at the human gate | Medium (Dependabot base rate) [minority: lane-2/primary-literature] | Medium | Low | 1-stub/run cap + dedupe against open stubs younger than the staleness window; UNTRIAGED stubs auto-stale at 30 days via harvest's filename-date sweep and re-enter the docket flagged `stub-stale` (mechanism specified §1.4/§2.3 round 1 — R1-22; round 0's "age out visibly" was policy-without-mechanism; human-set `graduation-queued` exempt — R2-11, so the inattention timer cannot cycle a good queued stub); cadence revisit trigger in §1.4 (R1-14) |
